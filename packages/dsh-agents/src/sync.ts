...
export async function readBaseComposition(baseDir: string, base: string): Promise<string | undefined> {
  try {
...
export async function materializePreset(
  ...
  const baseComposition = baseDir !== undefined ? await readBaseComposition(baseDir, baseId) : undefined
 ...
 * @param baseDir - the shipped preset root; absent uses the bare persona row.
 */
export async function syncPersonas(home: string, root: string, baseDir: string | undefined): Promise<SyncReport> {
