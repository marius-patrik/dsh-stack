window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function AgentsSection(props) {
			...
			useEffect(function () {
				var cancelled = false;
				listPresets().then(function (response) {
					if (cancelled) return;
			...
			if (error !== null) {
				return h("div", { style: { color: "var(--dsw-alias-state-error-primary)" } }, "Could not read the agent roster: " + error);
			}
			if (payload === null) {
				return h("div", { style: { color: "var(--dsw-alias-label-secondary)" } }, "Loading agents…");
			}
			var presets = payload.presets || [];
			var presetRow = function (preset) {
				var label = preset.name || preset.id;
				return h("div", { key: preset.id, style: { display: "flex", alignItems: "center", gap: "10px", border: "1px solid var(--dsw-alias-border-l2)", borderRadius: "10px", padding: "12px 14px" } },
					h("div", { style: { display: "grid", gap: "2px", minWidth: "0" } },
						h("div", { style: { display: "flex", alignItems: "center", gap: "8px" } },
							h("span", { style: { fontWeight: 600, fontSize: "14px" } }, label),
							preset.isDefault ? h("span", { style: { fontSize: "12px", color: "var(--dsw-alias-label-secondary)" } }, "default") : null,
							h("span", { style: { fontSize: "12px", color: "var(--dsw-alias-label-secondary)" } }, preset.trust)),
						preset.description ? h("div", { style: { fontSize: "13px", color: "var(--dsw-alias-label-secondary)" } }, preset.description) : null,
						preset.broken ? h("div", { style: { fontSize: "13px", color: "var(--dsw-alias-state-error-primary)" } }, preset.broken) : null));
			};
			return h("div", { style: { display: "grid", gap: "20px", maxWidth: "760px" } },
				h("div", null,
					h("h2", { style: { margin: "0 0 6px" } }, "Agents"),
					h("p", { style: { margin: 0, color: "var(--dsw-alias-label-secondary)" } }, "Persona files authored under the agent home are materialized into agent presets the harness roster discovers. This tab shows every preset the deployment composes.")),
				h("div", { style: { display: "grid", gap: "8px" } }, presets.map(presetRow)),
				h("div", { style: { fontSize: "13px", color: "var(--dsw-alias-label-secondary)" } }, presets.length + (presets.length === 1 ? " agent preset" : " agent presets")));
		}
		/**
		...
		function apply(ctx) {
			...
			}, AgentsSection), "dsh-agents: agents settings section");
			ctx.slots.inject("settings.section.icon", () => ctx.slots.register({
				name: "settings.section.icon",
				id: "agents",
				order: 0,
			}, AgentsGlyph), "dsh-agents: agents nav glyph");
			ctx.slots.inject("conversation.input.left", () => ctx.slots.register({
				name: "conversation.input.left",
				id: "persona-chip",
				inject: () => ({ nameFor: nameFor }),
			}, PersonaChip), "dsh-agents: persona composer badge");
			...
			ctx.inject(["commandUi", "sessions"], (scope) => {
				...
				scope.effect(() => command.register({
					...
					ui: {
						kind: "popupSelect",
						options: (session) => ensureRoster().then(function (entries) {
							var projection = sessions.get(session.sessionId).projections.get("persona");
							var current = projection && typeof projection.personaId === "string" ? projection.personaId : "";
						...
						onSelect: (option, session) => ctx.remote.commands.execute(session.sessionId, "/persona " + option.id)
