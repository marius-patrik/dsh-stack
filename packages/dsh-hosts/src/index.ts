...
declare module '@deepseek-ai/cordis' {
  interface Context { hosts: HostsService }
}
...
export class HostsService extends Service implements IHostsService {
  ...
  constructor(ctx: Context, public readonly config?: Partial<AccessConfig>) {
    super(ctx, 'hosts')
    ...
    if (server) {
      server.register({
        kind: 'exact', path: '/hosts/api/status',
      ...
      server.register({
        kind: 'exact', path: '/hosts/api/health',
        handler: async (_req: IncomingMessage, res: ServerResponse) => {
          res.writeHead(200, { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' })
      ...
      server.register({
        kind: 'exact', path: '/hosts/bootstrap.sh',
        handler: async (_req: IncomingMessage, res: ServerResponse) => {
      ...
      server.register({
        kind: 'exact', path: '/hosts/bootstrap.ps1',
        handler: async (_req: IncomingMessage, res: ServerResponse) => {
      ...
      server.register({
        kind: 'exact', path: '/hosts/api/sync/manifest',
        handler: async (_req: IncomingMessage, res: ServerResponse) => {
  ...
  getAccessConfig(): AccessConfig {
    ...
    return {
      mode, gatewayPort, backendPort,
      activeUrl: `http://${tailnetDns}:${gatewayPort}`,
      permanentUrl, tailnetDns, lanIp, clusterDomain: 'dsh.local',
    }
  }
