...
export class PackageManagersService {
    ...
    registerPackageTools() {
        ...
        tools.registerTool({
            ...
            description: 'Execute build, test, or custom scripts using the detected package manager (bun, pnpm, npm, cargo, uv)',
            parameters: {
                type: 'object',
                properties: {
                    script: { type: 'string', description: 'Script name or command to run (e.g. "build", "test", "lint")' },
        ...
        tools.registerTool({
            ...
            parameters: {
                type: 'object',
                properties: {
                    packages: { type: 'string', description: 'Package names separated by space (or empty for all deps)' },
            ...
            execute: async (params) => {
                ...
                let cmd = `${pm} install`;
                if (pm === 'bun')
                    cmd = `bun add ${isDev ? '-d ' : ''}${pkgs}`.trim();
                else if (pm === 'pnpm')
                    cmd = `pnpm add ${isDev ? '-D ' : ''}${pkgs}`.trim();
                else if (pm === 'yarn')
                    cmd = `yarn add ${isDev ? '--dev ' : ''}${pkgs}`.trim();
                else if (pkgs)
                    cmd = `npm install ${isDev ? '--save-dev ' : ''}${pkgs}`.trim();
        ...
        tools.registerTool({
            ...
            description: 'Check or activate required Node.js version from .nvmrc or specified version string',
            parameters: {
                type: 'object',
                properties: {
