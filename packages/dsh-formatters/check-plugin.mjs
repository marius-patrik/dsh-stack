...
assert.equal(formatterFor(undefined, { formatters: { '.ts': { argv: ['prettier'] } } }, '.ts').argv[0], 'prettier')
assert.equal(formatterFor({ formatters: { '.ts': { argv: ['black'] } } }, { formatters: { '.ts': { argv: ['prettier'] } } }, '.ts').argv[0], 'black')
...
assert.ok(registeredTools.some((t) => t.name === 'format'), 'format tool not registered')
...
const afterEdit = await hook(
  { name: 'edit', arguments: { file_path: workFile }, callId: 'c1', signal: new AbortController().signal },
  { isError: false },
  async () => ({ kind: 'accept' }),
)
...
const afterOther = await hook(
  { name: 'edit', arguments: { file_path: join(root, 'notes.txt') }, callId: 'c2', signal: new AbortController().signal },
  { isError: false },
  async () => ({ kind: 'accept' }),
)
