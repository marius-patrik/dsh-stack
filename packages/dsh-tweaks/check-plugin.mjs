...
const { readTweaksSection, writeTweaksSection, normalizeSection, sectionsEqual } = await import('./lib/mirror.js')
...
const base = {
  ...
  session: { planToggle: true, forkUndo: true, dragDropImages: true, maxImageBytes: 8 * 1024 * 1024 },
...
// v2: share renderer.
const eventLog = [
  JSON.stringify({ type: 'user/message', seq: 0, time: 1, data: { message: { content: 'hello <world>' } } }),
  JSON.stringify({ type: 'assistant/message', seq: 1, time: 2, data: { message: { content: 'hi there' } } }),
...
const respond = async (url) => {
  const res = { _status: 0, _body: '', writeHead(s) { this._status = s }, end(b) { this._body = b } }
...
const roRes = { _status: 0, _body: '', writeHead(s) { this._status = s }, end(b) { this._body = b } }
  ...
  tables: {
    sessions: {
      'session-abc': {
        ...
        rows: { sessionStats: { ver: 1, seq: 5, val: { turns: 3, steps: 9, llmMs: 1200, toolMs: 800, ttftMs: 100, decodeMs: 900, decodeTokens: 500 } } },
      },
    },
  },
}
...
assert.throws(() => sessionMod.validateKeybinds([
...
const forks = []
const sessionsStub = { create(_id, opts) { forks.push(opts); return {} } }
...
globalThis.window = { __ModuleLoader__: { load: (spec) => { globalThis.__clientSpec = spec } } }
...
const stubModules = {
  ...
  '@deepseek-ai/dsh-client-web-react': { bindSnapshotSelector: (observable) => (selector) => selector(observable.getSnapshot()) },
}
...
assert.deepEqual(clientModule.inject, ['slots', 'locale', 'layout', 'sessions', 'workspaces', 'connection'])
...
const slotsStub = {
  register(entry, component) {
    ...
    return () => { records.delete(entry.name) }
  },
  inject(name, fn) {
    const dispose = fn()
...
const ctxStub = {
  effect(fn) { fn(); return () => {} },
  on() { return () => {} },
  get(name) { if (name === 'connection') return connectionStub; throw new Error('unexpected get: ' + name) },
  locale: {
    register(ns, dicts) { localeEntries.set(ns, dicts) },
    bind(ns) {
      const dicts = localeEntries.get(ns)
      return (key) => (dicts && dicts.zh[key] !== undefined ? dicts.zh[key] : key)
    },
    getSnapshot() { return { revision: localeRevision } },
    subscribe() { return () => {} },
  },
...
const assertRegistered = (name, reason) => assert.ok(records.has(name), `${reason}: ${name} not registered`)
...
for (const hole of ['sidebar.workspaces', 'sidebar.settings', 'sidebar.footer.action', 'sidebar.newSession', 'sidebar.history']) {
...
for (const hole of ['settings.trigger', 'settings.header', 'settings.action', 'settings.close', 'settings.section', 'settings.onboarding', 'settings.section.icon']) {
  assert.ok(shellEntry.children[hole], `settings shell must declare ${hole}`)
}

const generalRec = allRecords.find((r) => r.entry.name === 'settings.section' && r.entry.id === 'general')
