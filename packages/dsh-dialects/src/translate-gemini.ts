 ...
 *   `gemini-<partIndex>`.
 */
export async function* translateGemini(payloads: AsyncIterable<string>): AsyncGenerator<StreamChunk> {
  ...
  for await (const payload of payloads) {
    ...
    try {
      ...
      throw new LlmError(`malformed NDJSON payload: ${payload.slice(0, 120)}`, 'MALFORMED_RESPONSE')
    }
