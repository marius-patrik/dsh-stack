...
assert.deepEqual(toolsFor(undefined, { tools: { a: { description: 'd', command: ['x'] } } }), { a: { description: 'd', command: ['x'] } })
assert.deepEqual(
  toolsFor({ tools: { a: { description: 'from-settings', command: ['s'] } } }, { tools: { a: { description: 'from-entry', command: ['e'] } } }),
...
assert.deepEqual(commandArgv({ command: ['node', '{script}', '--out', '{out}'] }, { script: 'a.js', out: 'b.txt' }), ['node', 'a.js', '--out', 'b.txt'])
...
const text0 = echoDef.output.render({ name: 'alice' }, { stdout: 'alice', stderr: '', exitCode: 0 })
...
{
  const addRes = spawnSync(process.execPath, [cli, 'add', 'lint-py', 'Lint a python file', 'ruff', 'check'], { env: cliEnv, encoding: 'utf8' })
  ...
  const rmRes = spawnSync(process.execPath, [cli, 'remove', 'lint-py'], { env: cliEnv, encoding: 'utf8' })
