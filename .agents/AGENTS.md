...
- Spread dispatched work across every configured, healthy provider account (`Settings → Accounts`, or `dsh --profile default vault list --json` when non-interactive), not the same one or two repeatedly — subscription and API accounts across every vendor, plus the aggregated "Other Providers" bucket (OpenRouter, Mistral, Groq, custom endpoints — OpenRouter alone reaches many more models, including Cerebras- and Z.ai/GLM-hosted ones, through one account). The chunk decomposition above and this spread work together: many small independent chunks on many different accounts route around any single provider or account's rate/usage limit, so a limit hit stalls only its own chunk rather than the batch. Within one dispatched run, `dsh`'s own same-vendor account rotation already recovers from a mid-task quota/rate-limit error onto a sibling account without losing progress (see the quota-consolidation work, #164) — this bullet is about the *initial* spread across a batch of dispatches, not manually babysitting an individual run.
