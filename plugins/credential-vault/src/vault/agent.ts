...
export class PrivilegedVaultCustodian {
  ...
  async reportAuthFailure(id: string, signal: AuthFailureSignal): Promise<AuthFailureOutcome> {
    ...
    await this.#log('failure_reported', record, id, outcome.kind === 'unknown_record' ? 'unknown' : `${outcome.kind}: ${outcome.reason}`)
    return outcome
  }
  ...
  async health(): Promise<CredentialHealth[]> {
    ...
    const permitted = new Set((await this.#accessibleDescriptors()).map((descriptor) => descriptor.id))
  ...
  async #authorized(id: string): Promise<SecretRecord> {
    ...
    if (!scopeAllows(record.scope, this.#identity)) {
      await this.#log('read_denied', record, id, `scope ${record.scope.workspace}/[${record.scope.agents.join(',')}]`)
      throw new VaultAccessError(id, this.#identity)
    }
  ...
  async #accessibleDescriptors(): Promise<SecretDescriptor[]> {
    ...
    for (const id of await this.#vault.list()) {
      const record = await this.#vault.get(id)
      if (record && scopeAllows(record.scope, this.#identity)) descriptors.push(descriptorOf(record))
    }
    return descriptors
  }
  ...
  async #log(action: AuditAction, record: SecretRecord | null, id: string | null, outcome: string): Promise<void> {
