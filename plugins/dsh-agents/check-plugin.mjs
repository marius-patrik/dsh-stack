...
const { basePresetDir, PRESET_ROOT, SOURCE_MARKER, syncPersonas, materializePreset } = await import('./lib/sync.js')
const { PersonaCatalog } = await import('./lib/catalog.js')
const { PERSONA_SELECTED, PersonaController, foldPersona, hasOpenTurn } = await import('./lib/controller.js')
...
const md = parsePersona('/x/reviewer.md', `---\nname: PR Reviewer\nbase: minimal\ndescription: Reviews pull requests\n---\n\nYou review PRs carefully.\nWith a second paragraph.`)
...
const json = parsePersona('/x/tester.json', JSON.stringify({ name: 'Tester', description: 'T', base: 'standard', prompt: 'Run the tests.' }))
assert.equal(json.id, 'tester')
assert.equal(json.prompt, 'Run the tests.')
assert.equal(json.base, 'standard')
assert.throws(() => parsePersona('/x/empty.json', JSON.stringify({ prompt: '   ' })), /must not be empty/)
...
assert.ok(row.includes("- id: persona\n  name: '@deepseek-ai/dsh-persona'\n  config:\n    text: ''"), 'neutral persona row: ' + row)
...
assert.ok(baseDir !== undefined && existsSync(join(baseDir, 'standard', 'agent.cordis.yml')), 'shipped standard preset must be reachable for the splice test')
...
assert.ok(!swapped.includes('You are a coding agent powered by the {{model}} model'), 'old persona prompt gone')
...
writeFileSync(join(homeRoot, 'reviewer.md'), `---\nname: PR Reviewer\nbase: standard\ndescription: Reviews pull requests\n---\n\nReview PRs.\n`)
writeFileSync(join(homeRoot, 'tester.json'), JSON.stringify({ name: 'Tester', prompt: 'Run tests.' }))
...
for (const id of ['reviewer', 'tester']) {
  assert.ok(existsSync(join(home, PRESET_ROOT, id, 'agent.cordis.yml')), `preset ${id} materialized`)
  assert.ok(existsSync(join(home, PRESET_ROOT, id, SOURCE_MARKER)), `preset ${id} marked`)
}
const reviewerComposition = readFileSync(join(home, PRESET_ROOT, 'reviewer', 'agent.cordis.yml'), 'utf8')
...
const bareComposition = readFileSync(join(bareHome, PRESET_ROOT, 'solo', 'agent.cordis.yml'), 'utf8')
...
writeFileSync(join(home, PRESET_ROOT, 'handmade', 'agent.cordis.yml'), '- id: persona\n  name: x\n')
...
const makeSession = (header = {}) => {
  const events = []
  return { events, header, append(type, data) { events.push({ type, data }) } }
}

// foldPersona
assert.equal(foldPersona([]), '')
assert.equal(foldPersona([{ type: 'other' }]), '')
assert.equal(foldPersona([{ type: PERSONA_SELECTED, data: { personaId: 'reviewer' } }]), 'reviewer')
assert.equal(foldPersona([
...
assert.equal(plugin.personaPolicyText({ agent: { session: s3 } }, controller, ctrlCatalog, undefined), 'You review PRs.', 'live wins')
const s4 = makeSession()
assert.equal(plugin.personaPolicyText({ agent: { session: s4 } }, controller, ctrlCatalog, undefined), '', 'no live, no header, no fallback → empty')
const s5 = makeSession()
assert.equal(plugin.personaPolicyText({ agent: { session: s5 } }, controller, ctrlCatalog, 'reviewer'), 'You review PRs.', 'fallback wins')
const s6 = makeSession()
assert.equal(plugin.personaPolicyText({ agent: { session: { events: [], header: { agentPreset: 'reviewer' }, append() {} } } }, controller, ctrlCatalog, undefined), 'You review PRs.', 'header wins')
const s7 = makeSession()
assert.equal(plugin.personaPolicyText({ agent: { session: { events: [], header: { agentPreset: 'unknown' }, append() {} } } }, controller, ctrlCatalog, undefined), '', 'unknown header → empty')
assert.equal(plugin.personaPolicyText({}, controller, ctrlCatalog, undefined), '', 'no agent → empty')
...
actx.provide('settings', {
  ...
  register(_ns, _schema, opts) { if (!sections.has(_ns)) sections.set(_ns, opts.base); return { get: (ns) => sections.get(ns), watch: () => undefined } },
})
actx.provide('systemPrompt', { section: (def) => { capturedSection = def; return () => {} } })
actx.provide('sessionProjections', { register: (def) => { capturedProjection = def; return () => {} } })
actx.provide('commands', { register: (def) => { capturedCommand = def; return () => {} } })
actx.logger = { info: () => {}, warn: (m) => console.log('WARN:', m) }
plugin.apply(actx, {})
await new Promise((resolve) => setTimeout(resolve, 500))

// boot sync: neutral row in the materialized preset
assert.ok(existsSync(join(bootHome, PRESET_ROOT, 'boot', SOURCE_MARKER)), 'boot sync materialized the persona')
assert.ok(existsSync(join(bootHome, PRESET_ROOT, 'boot', 'agent.cordis.yml')))
const bootComposition = readFileSync(join(bootHome, PRESET_ROOT, 'boot', 'agent.cordis.yml'), 'utf8')
...
assert.equal(capturedSection.text({ agent: { session: bootAgentSession } }), 'Boot persona.', 'header preset resolves via catalog')
...
assert.deepEqual(
  capturedProjection.apply({ personaId: '', pending: false }, { type: 'command/run', data: { name: 'persona', args: 'boot' } }),
  { personaId: 'boot', pending: true },
  'command/run → pending',
)
assert.deepEqual(
  capturedProjection.apply({ personaId: 'boot', pending: true }, { type: PERSONA_SELECTED, data: { personaId: 'boot' } }),
...
const remove = spawnSync(process.execPath, [cli, 'remove', 'writer'], { env: cliEnv, encoding: 'utf8' })
assert.equal(remove.status, 0, remove.stderr)
assert.ok(remove.stdout.includes('removed persona writer'))
assert.ok(!existsSync(join(cliHome, PRESET_ROOT, 'writer')), 'removing the persona pruned its preset')
const missing = spawnSync(process.execPath, [cli, 'remove', 'nope'], { env: cliEnv, encoding: 'utf8' })
...
assert.ok(readFileSync(new URL('./client.js', import.meta.url), 'utf8').includes('__ModuleLoader__.load'))
const loader = {}
globalThis.window = { __ModuleLoader__: { load: (spec) => { loader.spec = spec } } }
...
const clientExports = loader.spec.factory((spec) => {
  ...
  if (spec === '@deepseek-ai/dsh-client-ui-primitives') return { IconGoalOutline16: () => null, IconPersonalizationOutline16: () => null }
  throw new Error('unexpected require: ' + spec)
}, {})
assert.deepEqual(clientExports.inject, ['slots', 'connection', 'commandUi', 'sessions', 'remote'], 'client inject must match package.json')
const clientRegistrants = new Map()
const clientCtx = {
  effect(fn) { fn() },
  slots: {
    inject(name, fn) { clientRegistrants.set(name, fn) },
  ...
  inject(services, cb) {
    const scope = {
      effect(fn) { fn() },
      get(name) {
        if (name === 'commandUi') return { register(spec) { commandUiRegistrants.push(spec); return () => {} } }
        return undefined
      },
      sessions: {
        get() { return { projections: { get() { return currentProjection } } } },
      },
    }
    cb(scope)
  },
  connection: {
    api: { agentPresets: { list() { return Promise.resolve({ result: { ok: true, value: { presets: stubRoster } } }) } } },
  },
  remote: {
    commands: {
      execute(sessionId, line) { executedCommands.push({ sessionId, line }); return Promise.resolve({ ok: true }) },
    },
  },
}
