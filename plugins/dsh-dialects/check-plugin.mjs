...
assert.deepEqual(ids, ['openai', 'claude', 'gemini', 'code-assist', 'antigravity'], 'unexpected dialect registry')
...
assert.throws(() => assist.serialize({ provider: 'p', model: 'm', messages: [] }, {}, 'https://cloudcode-pa.googleapis.com/v1internal', { maxTokens: 10 }), /bearer token/)
const assistRequest = assist.serialize(
  { provider: 'p', model: 'gemini-3.6-flash', system: 'be terse', messages: [{ role: 'user', content: [{ type: 'text', text: 'hi' }] }] },
  { token: 't' },
  'https://cloudcode-pa.googleapis.com/v1internal',
  { maxTokens: 10 },
)
assert.equal(assistRequest.url, 'https://cloudcode-pa.googleapis.com/v1internal:streamGenerateContent?alt=sse')
...
assert.ok(typeof wrapped.request.session_id === 'string' && wrapped.request.session_id.length === 36)
