 ...
 * `ctx.dialects` service and the bundled `openai`, `claude`, and `gemini`
 * dialects; LLM adapter plugins (e.g. dsh-providers) resolve a dialect by id
 * to serialize requests and translate response streams.
 *
 * @module dsh-dialects
 */
...
export function apply(ctx: Context, _config: Config): void {
  ...
  for (const dialect of [openaiDialect, claudeDialect, geminiDialect]) {
    ctx.effect(() => {
