...
import { antigravityDialect, splitConversation, ANTIGRAVITY_PROJECT_HEADER } from './lib/antigravity.js'
...
function assistantText(text) {
  return { id: id(), role: 'assistant', content: [{ type: 'text', text }], source: { kind: 'model', provider: 'p', model: 'm' } }
}
...
const req = openaiDialect.serialize(
  { provider: 'p', model: 'deepseek', system: 'be brief', messages: [userMessage('hi')], tools: [{ name: 't', description: 'a', parameters: { type: 'object' } }], temperature: 0.3, maxTokens: 64, stop: ['END'] },
  { apiKey: 'k' },
...
assert.deepEqual(types, ['block-start', 'reasoning-delta', 'block-start', 'text-delta', 'block-start', 'tool-call-delta', 'tool-call-delta', 'tool-call-delta', 'block-end', 'block-end', 'block-end', 'usage', 'finish'])
...
{
  const base = { model: 'claude-opus-5', messages: [{ role: 'user', content: [{ type: 'text', text: 'hi' }] }] }
  const defaults = { maxTokens: 1024 }

  const oauth = claudeDialect.serialize(base, { token: 'oauth-token' }, 'https://api.anthropic.com/v1', defaults)
  ...
  const withSystem = claudeDialect.serialize(
    { ...base, system: 'Be terse.' }, { token: 'oauth-token' }, 'https://api.anthropic.com/v1', defaults,
  )
  ...
  assert.equal(JSON.parse(
    claudeDialect.serialize(base, { apiKey: 'sk-test' }, 'https://api.anthropic.com/v1', defaults).body,
  ).system, undefined)
  assert.equal(JSON.parse(
    claudeDialect.serialize({ ...base, system: 'Be terse.' }, { apiKey: 'sk-test' }, 'https://api.anthropic.com/v1', defaults).body,
  ).system, 'Be terse.')
...
// --- antigravity: the subscription transport, not the Code Assist one ---
{
  ...
  const req = antigravityDialect.serialize(opts, auth, 'https://cloudcode-pa.googleapis.com/v1internal', { maxTokens: 64 })
  ...
  assert.deepEqual(body.history, [{ content: 'Be terse.' }, { content: 'first' }, { content: 'answer' }])
  ...
  assert.throws(() => antigravityDialect.serialize(opts, { token: 'tok' }, 'https://x/v1internal', { maxTokens: 64 }), /ANTIGRAVITY_PROJECT/)
  assert.throws(() => antigravityDialect.serialize(opts, { headers: auth.headers }, 'https://x/v1internal', { maxTokens: 64 }), /no bearer token/)
  assert.throws(() => splitConversation({ model: 'm', messages: [{ role: 'assistant', content: [{ type: 'text', text: 'a' }] }] }), /trailing user message/)

  const wire = JSON.stringify([
    { markdown: 'wor' },
    { markdown: 'king', usageMetadata: { candidatesTokenCount: '2', totalTokenCount: '10' } },
  ])
  const stream = new ReadableStream({ start(c) { c.enqueue(new TextEncoder().encode(wire)); c.close() } })
  const chunks = []
  for await (const chunk of antigravityDialect.parse(stream)) chunks.push(chunk)
  assert.equal(chunks.filter(c => c.type === 'text-delta').map(c => c.text).join(''), 'working')
  ...
  const empty = new ReadableStream({ start(c) { c.enqueue(new TextEncoder().encode('[]')); c.close() } })
  const emptyChunks = []
...
const events = [
  { event: 'message_start', data: JSON.stringify({ type: 'message_start', message: { id: 'msg_1', usage: { input_tokens: 8 } } }) },
  { event: 'content_block_start', data: JSON.stringify({ type: 'content_block_start', index: 0, content_block: { type: 'tool_use', id: 'tu_1', name: 'search', input: {} } }) },
  { event: 'content_block_delta', data: JSON.stringify({ type: 'content_block_delta', index: 0, delta: { type: 'input_json_delta', partial_json: '{"q":' } }) },
  { event: 'content_block_delta', data: JSON.stringify({ type: 'content_block_delta', index: 0, delta: { type: 'input_json_delta', partial_json: '"x"}' } }) },
...
assert.deepEqual(ctypes, ['block-start', 'tool-call-delta', 'tool-call-delta', 'tool-call-delta', 'block-end', 'usage', 'finish'])
...
const glines = [
  ...
  { candidates: [{ content: { parts: [{ text: 'Hello world' }, { functionCall: { name: 'search', args: { q: 'x' } } }] } }] },
  { candidates: [{ content: { parts: [{ text: 'Hello world' }, { functionCall: { name: 'search', args: { q: 'x' } } }] }, finishReason: 'STOP' }], usageMetadata: { promptTokenCount: 4, candidatesTokenCount: 2 } },
]
const gchunks = await collect(translateGemini(glines.map(l => JSON.stringify(l))))
const gtypes = gchunks.map(c => c.type)
assert.deepEqual(gtypes, ['block-start', 'text-delta', 'text-delta', 'block-start', 'tool-call-delta', 'block-end', 'block-end', 'usage', 'finish'])
...
const toolCallMsg = { id: id(), role: 'assistant', content: [{ type: 'tool-call', id: 'call_9', name: 'search', arguments: '{"q":"x"}' }], source: { kind: 'model', provider: 'p', model: 'm' } }
const toolResultMsg = { id: id(), role: 'user', content: [{ type: 'tool-result', toolCallId: 'call_9', content: [{ type: 'text', text: 'r1' }] }], source: { kind: 'tool', callId: 'call_9' } }
