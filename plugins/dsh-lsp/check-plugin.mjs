...
assert.deepEqual(
  plugin.mergeServers({ a: { command: 'x', extensionToLanguage: { '.x': 'x' } } },
...
assert.deepEqual(
  plugin.mergeServers({ a: { command: 'x', extensionToLanguage: { '.x': 'x' } } },
    { servers: { a: { command: 'z', extensionToLanguage: { '.a': 'a' } } } }).a.command,
  'z',
)
...
const sections2 = new Map([[NS, { servers: {
  typescript: { command: 'typescript-language-server', extensionToLanguage: { '.ts': 'typescript' }, args: ['--stdio'] },
} }]])
...
execFileSync(process.execPath, [lspCli, 'servers', 'add',
  'typescript', 'typescript-language-server', '--ext=.ts=typescript', '--ext=.tsx=typescriptreact',
