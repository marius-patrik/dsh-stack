...
import { MemoryAuditLog, PrivilegedVaultCustodian, VaultAccessError, VaultMaterialSealedError } from './lib/vault/agent.js'
import { classifyAuthFailure, planReauth, ReauthSupervisor } from './lib/vault/supervisor.js'
import { findProviderDescriptor, registerOAuthSupplement, registerProviderRoutes } from './lib/vault/provider-descriptor.js'
...
const credentialsFiber = await ctx.plugin(LocalCredentialProvider, { path: credentialFile, watch: false })
...
assert.deepEqual(claudeImports.map((r) => r.ref).sort(), ['CLAUDE_API_KEY', 'CLAUDE_SUB_OAUTH_TOKEN'])
...
console.log('claude importer ok:', claudeImports.map((r) => r.ref))
...
assert.ok(cursorImports.some((r) => r.ref === 'CURSOR_SUB_TOKEN'), 'cursor token not imported')
assert.equal((await ctx.accounts.resolve('CURSOR_SUB_TOKEN')).value, 'cursor-jwt-token')
console.log('cursor importer ok:', cursorImports.map((r) => r.ref))
...
assert.deepEqual(ghImports.map((r) => r.ref).sort(), ['GITHUB_ENTERPRISE_HOST', 'GITHUB_ENTERPRISE_TOKEN', 'GITHUB_OAUTH_TOKEN', 'GITHUB_USER'])
...
assert.deepEqual(await migratedCtx.accounts.resolve('CLAUDE_SUB_OAUTH_TOKEN'), { value: 'migrated-token', origin: 'vault' })
assert.deepEqual(await migratedCtx.accounts.resolve('KIMI_API_KEY'), { value: 'migrated-key', origin: 'vault' })
assert.ok(!existsSync(join(legacyHome, 'accounts.vault')), 'legacy document not retired')
assert.ok(existsSync(join(legacyHome, 'accounts.vault.v1-migrated')), 'legacy document not preserved')
...
/* -------------------------------------------------------------------------- */
{
  ...
  const materials = [
    ...
    { type: 'password', username: 'user@example.com', password: new SecretValue('pw-1'), origin: null, loginUrl: null },
    {
      type: 'totp_seed',
      parameters: createTotpParameters({ secret: 'JBSWY3DPEHPK3PXP', algorithm: 'SHA1', digits: 6, period: 30, issuer: 'GitHub', account: 'user' }),
    },
    {
      type: 'passkey',
    ...
    { type: 'cookie_jar', origin: 'https://example.com', jar: new SecretValue('session=abc; sid=123'), sessionExpiresAt: null },
    { type: 'recovery_codes', codes: [new SecretValue('AAAA-1111'), new SecretValue('BBBB-2222')], consumed: 0 },
    {
      type: 'ssh_key',
  ...
  const encrypted = new EncryptedFileVault({ directory: join(home, 'roundtrip'), masterKey: new StaticMasterKey(randomBytes(32)) })
  ...
  for (const material of materials) {
    ...
    const record = createSecretRecord({ id, label: material.type, purpose: 'demo', scope, material, now: () => now })
    await memory.put(record)
    await encrypted.put(record)
    assert.ok(materialEquals(material, (await memory.get(id)).material), `memory round-trip failed for ${material.type}`)
    assert.ok(materialEquals(material, (await encrypted.get(id)).material), `encrypted round-trip failed for ${material.type}`)
  ...
  const vaultDoc = readdirSync(join(home, 'roundtrip')).map((file) => readFileSync(join(home, 'roundtrip', file), 'utf8')).join('\n')
  ...
  const rotated = rotateSecretRecord(await encrypted.get('rt-api-key'), { type: 'api_key', apiKey: new SecretValue('sk-test-2'), header: null }, { now: () => now + 1000 })
  ...
  await assert.rejects(async () => {
    ...
    rotateSecretRecord(record, { type: 'password', username: 'u', password: new SecretValue('x'), origin: null, loginUrl: null })
  })
...
/* -------------------------------------------------------------------------- */
{
  ...
  const vectors = [
    { algorithm: 'SHA1', seed: '12345678901234567890', cases: [[59, '94287082'], [1111111109, '07081804'], [1111111111, '14050471'], [1234567890, '89005924']] },
    { algorithm: 'SHA256', seed: '12345678901234567890123456789012', cases: [[59, '46119246'], [1111111109, '68084774'], [1111111111, '67062674'], [1234567890, '91819424']] },
    { algorithm: 'SHA512', seed: '1234567890123456789012345678901234567890123456789012345678901234', cases: [[59, '90693936'], [1111111109, '25091201'], [1111111111, '99943326'], [1234567890, '93441116']] },
  ]
  for (const { algorithm, seed, cases } of vectors) {
    const key = encoder.encode(seed)
    const parameters = createTotpParameters({ secret: encodeBase32(key), algorithm, digits: 8, period: 30 })
  ...
  assert.equal(hotpCode(encoder.encode(vectors[0].seed), 1, { digits: 8, algorithm: 'SHA1' }), '94287082')
  ...
  assert.throws(() => parseOtpauthUri('otpauth://hotp/ACME:user?secret=JBSWY3DPEHPK3PXP&counter=1'))
...
/* -------------------------------------------------------------------------- */
{
  ...
  const reopen = new EncryptedFileVault({ directory: dir, masterKey: new KeyFileMasterKey({ directory: dir }) })
  assert.equal((await reopen.get('mk-secret')).material.apiKey.reveal(), 'mk-secret-value')
  await assert.rejects(new EncryptedFileVault({ directory: dir, masterKey: new StaticMasterKey(randomBytes(32)) }).get('mk-secret'))
  ...
  await new EncryptedFileVault({ directory: ppDir, masterKey: new PassphraseMasterKey({ directory: ppDir, passphrase: new SecretValue('correct horse'), scrypt: params }) }).put(ppRecord)
  const ppReopen = new EncryptedFileVault({ directory: ppDir, masterKey: new PassphraseMasterKey({ directory: ppDir, passphrase: new SecretValue('correct horse'), scrypt: params }) })
  assert.equal((await ppReopen.get('pp-secret')).material.note.reveal(), 'pp-secret-value')
  await assert.rejects(
    new EncryptedFileVault({ directory: ppDir, masterKey: new PassphraseMasterKey({ directory: ppDir, passphrase: new SecretValue('wrong horse'), scrypt: params }) }).get('pp-secret'),
  )
...
/* -------------------------------------------------------------------------- */
{
  ...
  await vault.put(createSecretRecord({ id: 'owner-secret', label: 'Owner', purpose: 'demo', scope: { workspace: '*', agents: [] }, material: { type: 'generic_note', note: new SecretValue('owner-material') }, now: () => now }))
  await vault.put(createSecretRecord({ id: 'agent-secret', label: 'Agent', purpose: 'demo', scope: { workspace: '*', agents: ['helper'] }, material: { type: 'generic_note', note: new SecretValue('agent-material') }, now: () => now }))
  await vault.put(createSecretRecord({ id: 'totp', label: 'GitHub', purpose: 'github', scope: { workspace: '*', agents: ['helper'] }, material: { type: 'totp_seed', parameters: createTotpParameters({ secret: 'JBSWY3DPEHPK3PXP' }) }, now: () => now }))
  const audit = new MemoryAuditLog()
  const custodian = new PrivilegedVaultCustodian({ vault, identity: { workspace: 'w', agent: 'helper' }, audit, now: () => now })
  ...
  assert.deepEqual((await custodian.listAccessible()).map((descriptor) => descriptor.id).sort(), ['agent-secret', 'totp'])
...
/* -------------------------------------------------------------------------- */
{
  assert.deepEqual(classifyAuthFailure({ transport: true }), { kind: 'recoverable', reason: 'transport_failure' })
  assert.deepEqual(classifyAuthFailure({ status: 401, error: 'invalid_grant' }), { kind: 'terminal', reason: 'invalid_grant' })
  assert.deepEqual(classifyAuthFailure({ status: 401 }), { kind: 'terminal', reason: 'unknown' })
  assert.deepEqual(classifyAuthFailure({ status: 429 }), { kind: 'recoverable', reason: 'rate_limited' })
  assert.deepEqual(classifyAuthFailure({ status: 503 }), { kind: 'recoverable', reason: 'http_503' })
  assert.deepEqual(classifyAuthFailure({ message: 'a captcha was shown' }), { kind: 'terminal', reason: 'captcha_required' })

  const now = 1_700_000_000_000
  const dead = createSecretRecord({ id: 'dead-oauth', label: 'API', purpose: 'demo', scope: { workspace: '*', agents: [] }, material: { type: 'oauth_token', accessToken: new SecretValue('at'), refreshToken: new SecretValue('rt'), refreshTokenExpiresAt: null, scopes: [], subscriptionType: null, tokenEndpoint: null }, now: () => now })
  const password = createSecretRecord({ id: 'pw-peer', label: 'Password', purpose: 'demo', scope: { workspace: '*', agents: [] }, material: { type: 'password', username: 'u', password: new SecretValue('pw'), origin: null, loginUrl: null }, now: () => now })
  const totpSeed = createSecretRecord({ id: 'totp-peer', label: 'Totp', purpose: 'demo', scope: { workspace: '*', agents: [] }, material: { type: 'totp_seed', parameters: createTotpParameters({ secret: 'JBSWY3DPEHPK3PXP' }) }, now: () => now })
  ...
  assert.equal(planReauth(dead, 'captcha_required', [password, totpSeed]).strategy, 'human_presence_required')
  ...
  const oauth = createSecretRecord({
    ...
    material: { type: 'oauth_token', accessToken: new SecretValue('old-access'), refreshToken: new SecretValue('old-refresh'), refreshTokenExpiresAt: null, scopes: ['openid'], subscriptionType: null, tokenEndpoint: null },
  ...
  const transport = async (request) => {
    ...
    return { status: 200, body: { access_token: 'fresh-access', refresh_token: 'fresh-refresh', expires_in: 3600, token_type: 'Bearer' } }
  }
  const authFor = async () => ({ method: 'oauth_pkce', authorizeUrl: 'https://auth.example/authorize', tokenUrl: 'https://token.example/token', clientId: 'client-1', scopes: ['openid'], redirect: 'loopback' })
  const supervisor = new ReauthSupervisor({ vault, now: () => now, transport, random: () => 0.5, sleep: async () => {}, authFor })
  ...
  const failure = await supervisor.reportFailure('svc-oauth', { status: 401, error: 'invalid_grant' })
  ...
  assert.equal(await supervisor.ensureFresh('svc-oauth').then((outcome) => outcome.kind), 'reauth_required')
...
/* -------------------------------------------------------------------------- */
{
  ...
  assert.equal(await vaultCommand(['add', '--id', 'cli-token', '--type', 'api_key', '--label', 'CLI token', '--purpose', 'demo', '--stdin'], io), 0)
  ...
  assert.ok(captured.some((line) => line.includes('apiKey=cli-…(16)')), 'get must show a fingerprint, not the value')
  assert.ok(!captured.join('').includes('cli-secret-value'))
  captured.length = 0
  assert.equal(await vaultCommand(['get', '--id', 'cli-token', '--reveal'], io), 0)
  assert.ok(captured.some((line) => line.includes('cli-secret-value')), '--reveal is the only door')
  captured.length = 0
  const revealFile = join(home, 'cli-reveal.txt')
  assert.equal(await vaultCommand(['get', '--id', 'cli-token', '--reveal', '--out', revealFile], io), 0)
  assert.ok(!captured.join('').includes('cli-secret-value'))
  ...
  assert.equal(await vaultCommand(['add', '--id', 'argv-token', '--type', 'api_key', '--label', 'A', '--purpose', 'demo', '--value', 'argv-secret-xyz'], io), 1)
  ...
  assert.deepEqual(fingerprint('apiKey', 'sk-abcdef1234'), { field: 'apiKey', prefix: 'sk-a', length: 13 })
...
/* -------------------------------------------------------------------------- */
{
  ...
  await store.put('gh-oauth', { kind: 'oauth', accessToken: new SecretValue('at-value'), refreshToken: new SecretValue('rt-value'), expiresAt: null, refreshTokenExpiresAt: null, scopes: ['repo'], subscriptionType: null, obtainedAt: now })
  ...
  await vault.put(createSecretRecord({ id: 'plain-password', label: 'Password', purpose: 'demo', scope: { workspace: '*', agents: [] }, material: { type: 'password', username: 'u', password: new SecretValue('pw'), origin: null, loginUrl: null } }))
  assert.deepEqual(await store.list(), ['gh-oauth'])
  // A record the store auto-created carries a scope that denies every agent.
  const custodian = new PrivilegedVaultCustodian({ vault, identity: { workspace: 'w', agent: 'x' } })
...
/* -------------------------------------------------------------------------- */
{
  ...
  for (const ref of ['CLAUDE_SUB_OAUTH_TOKEN', 'OPENAI_API_KEY', 'GITHUB_OAUTH_TOKEN', 'DEEPSEEK_API_KEY']) {
...
/* -------------------------------------------------------------------------- */
{
  let routes = null
  try {
    const lib = process.env.DSH_PROVIDERS_PROVIDERS_LIB ?? '/Users/user/agents/plugins/dsh-providers/lib/providers.js'
  ...
  if (routes) {
    ...
    for (const ref of refs) assert.ok(list.stdout.includes(slugRecordId(ref)), `dsh accounts list missing ${slugRecordId(ref)}`)
    const reveal = spawnSync(process.execPath, [bin, 'get', '--id', slugRecordId('CLAUDE_SUB_OAUTH_TOKEN'), '--reveal'], { env, encoding: 'utf8' })
    assert.equal(reveal.status, 0, reveal.stderr)
    assert.ok(reveal.stdout.includes('boot-CLAUDE_SUB_OAUTH_TOKEN'), 'owner CLI did not see the service-written token')
    console.log('boot witness ok — dsh-providers resolve() and `dsh accounts` share one vault:', [...refs].join(', '))
  }
}
...
function firstSecret(material) {
  switch (material.type) {
    case 'api_key': return material.apiKey.reveal()
    case 'oauth_token': return material.accessToken.reveal()
    case 'password': return material.password.reveal()
    case 'totp_seed': return material.parameters.secret.reveal()
    case 'passkey': return material.privateKey.reveal()
    case 'cookie_jar': return material.jar.reveal()
    case 'recovery_codes': return material.codes[0]?.reveal() ?? null
    case 'ssh_key': return material.privateKey.reveal()
    case 'generic_note': return material.note.reveal()
  }
}
...
function materialEquals(a, b) {
  ...
  switch (a.type) {
    case 'api_key': return reveal(a.apiKey) === reveal(b.apiKey) && a.header === b.header
    case 'oauth_token':
      return (
        reveal(a.accessToken) === reveal(b.accessToken) &&
        (a.refreshToken ? reveal(a.refreshToken) : null) === (b.refreshToken ? reveal(b.refreshToken) : null) &&
    ...
    case 'password': return a.username === b.username && reveal(a.password) === reveal(b.password) && a.origin === b.origin && a.loginUrl === b.loginUrl
    case 'totp_seed':
      return (
    ...
    case 'cookie_jar': return a.origin === b.origin && reveal(a.jar) === reveal(b.jar) && a.sessionExpiresAt === b.sessionExpiresAt
    case 'recovery_codes':
      return a.consumed === b.consumed && a.codes.length === b.codes.length && a.codes.every((code, index) => reveal(code) === reveal(b.codes[index]))
    case 'ssh_key':
      return (
        reveal(a.privateKey) === reveal(b.privateKey) &&
        a.publicKey === b.publicKey &&
        (a.passphrase ? reveal(a.passphrase) : null) === (b.passphrase ? reveal(b.passphrase) : null) &&
        a.fingerprint === b.fingerprint &&
        a.comment === b.comment
      )
    case 'generic_note': return reveal(a.note) === reveal(b.note)
  }
  return false
}
...
assert.equal(readFileSync(clientPath, 'utf8'), readFileSync(join(dirname(fileURLToPath(import.meta.url)), 'client.js'), 'utf8'), 'lib/client.js must be the built copy of client.js')
const registered = {}
globalThis.window = { __ModuleLoader__: { load: (spec) => { registered.spec = spec } } }
...
const clientCtx = {
  ...
  locale: { register() {} },
  slots: {
    inject(name, fn) { registrants.set(name, fn) },
    register(spec) { registrations.push(spec); return spec },
  },
}
...
// ---- CLI login: launcher resolution + keychain shapes ----
{
  ...
  const native = login.resolveCliInvocation('/Users/user/.npm-global/bin/claude', ['auth', 'login'])
  ...
  const goKeyring = 'go-keyring-base64:' + Buffer.from(JSON.stringify({
    token: { access_token: 'gem-access', refresh_token: 'gem-refresh', expiry: '2026-01-01T00:00:00Z' },
  })).toString('base64')
  assert.deepEqual(login.parseKeychainCredentials(goKeyring), {
    accessToken: 'gem-access', refreshToken: 'gem-refresh', expiresAt: '2026-01-01T00:00:00Z',
  })
  assert.deepEqual(login.parseKeychainCredentials(JSON.stringify({
    claudeAiOauth: { accessToken: 'cc-access', refreshToken: 'cc-refresh', expiresAt: 1787077129647 },
