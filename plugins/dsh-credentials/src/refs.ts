...
import { createSecretRecord, type SecretDescriptor, type SecretMaterial, type SecretRecord, type SecretType } from './vault/record.js'
...
export function purposeForRef(ref: string): string {
  return REF_PURPOSES[ref] ?? (ref.startsWith('GEMINI_SUB_COOKIE_') ? 'google/gemini-cli' : slugRecordId(ref))
}
...
export function materialFromValue(ref: string, value: string): SecretMaterial {
  switch (refMaterialKind(ref)) {
    ...
    case 'cookie_jar':
      return { type: 'cookie_jar', origin: slugRecordId(ref), jar: new SecretValue(value), sessionExpiresAt: null }
    case 'oauth_token':
      return {
        type: 'oauth_token',
 ...
 */
export function recordForRef(ref: string, value: string, options: { now?: () => number; account?: string } = {}): SecretRecord {
...
export function canonicalRefsForPurpose(purpose: string, type: SecretType): readonly string[] {
  ...
  if (purpose === 'anthropic/claude-code' || purpose.startsWith('anthropic/claude-code-')) return ['CLAUDE_SUB_OAUTH_TOKEN']
