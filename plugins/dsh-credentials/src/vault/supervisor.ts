...
import { descriptorOf, effectiveExpiryMs, type SecretRecord, type SecretScope, type SecretType } from './record.js'
...
export type OAuthAuthResolver = (record: OAuthTokenRecord) => OAuthAuthConfig | null | Promise<OAuthAuthConfig | null>
...
const IMPLEMENTED_STRATEGIES: ReadonlySet<ReauthStrategy> = new Set<ReauthStrategy>(['oauth_refresh'])
...
export function classifyAuthFailure(signal: AuthFailureSignal): FailureClassification {
  ...
  return { kind: 'recoverable', reason: code || (status === null ? 'unclassified' : `http_${status}`) }
}
 ...
 */
export function planReauth(record: SecretRecord, reason: TerminalReason, peers: readonly SecretRecord[]): ReauthPlan {
  ...
  const recovery = family.find(
    (peer) => peer.type === 'recovery_codes' && peer.material.type === 'recovery_codes' && peer.material.codes.length > peer.material.consumed,
  )
  ...
  if (passkey && passkey.material.type === 'passkey' && !passkey.material.userVerificationRequired) {
    return {
  ...
  if (recovery) {
    return {
      ...
      humanPresenceRequired: false,
      automatedToday: false,
      caveat: 'recovery codes are single use and finite; spending one without replacing it moves the failure later, not away',
    }
  }

  return human(record, reason, `no password, passkey, or recovery code is stored for ${record.purpose}`)
}
...
function reasonCaveat(reason: TerminalReason): string | null {
  return reason === 'unknown' ? 'the provider gave no machine-readable reason; the classification is a fallback' : null
}
...
export class ReauthSupervisor {
  ...
  constructor(options: ReauthSupervisorOptions) {
    ...
    this.#refresher = options.refresher ?? new OAuthTokenRefresher({ ...refresherOptions, refreshSkewMs: this.#refreshSkewMs })
  ...
  async sweep(): Promise<SweepResult> {
    ...
    return { checkedAt: new Date(this.#now()).toISOString(), refreshed, retrying, reauthRequired, healthy }
  }
  ...
  async ensureFresh(id: string): Promise<EnsureFreshOutcome> {
    ...
    if (terminal?.terminalReason) {
      return { kind: 'reauth_required', record, event: await this.#raise(record, terminal.terminalReason) }
    }
  ...
  async reportFailure(id: string, signal: AuthFailureSignal): Promise<AuthFailureOutcome> {
    ...
    if (classification.kind === 'terminal') {
      return { kind: 'terminal', reason: classification.reason, event: await this.#raise(record, classification.reason) }
    }
    const attempt = this.#recordRecoverable(id, classification.reason)
    return { kind: 'recoverable', reason: classification.reason, retryAfterMs: this.#backoff(attempt - 1), attempt }
  }
  ...
  async reauth(id: string): Promise<EnsureFreshOutcome> {
    ...
    if (record.type !== 'oauth_token') {
      return { kind: 'reauth_required', record, event: await this.#raise(record, 'expired_without_refresh_path') }
    }
  ...
  async #refreshOAuth(record: OAuthTokenRecord, options: { force?: boolean } = {}): Promise<EnsureFreshOutcome> {
    if (!record.material.refreshToken) {
      return { kind: 'reauth_required', record, event: await this.#raise(record, 'no_refresh_token') }
    }
    if (isPast(record.material.refreshTokenExpiresAt, this.#now())) {
      return { kind: 'reauth_required', record, event: await this.#raise(record, 'refresh_token_expired') }
    }
    const auth = await this.#authFor(record)
    if (!auth) {
      return { kind: 'reauth_required', record, event: await this.#raise(record, 'expired_without_refresh_path') }
    }
    const refresher = options.force && this.#forcedRefresher ? this.#forcedRefresher : this.#refresher
  ...
  #healthOf(record: SecretRecord, all: readonly SecretRecord[], nowMs: number): CredentialHealth {
    ...
    return {
      ...
      selfHealing: state !== 'needs_reauth' && refreshable && IMPLEMENTED_STRATEGIES.has(plan.strategy),
  ...
  #recordRecoverable(id: string, reason: string): number {
    ...
    this.#failures.set(id, { attempts, lastFailureAt: this.#now(), lastReason: reason, terminalReason: null })
    return attempts
  }
