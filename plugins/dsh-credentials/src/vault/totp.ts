...
export function createTotpParameters(input: TotpParametersInput): TotpParameters {
  ...
  if (!TOTP_ALGORITHMS.includes(algorithm)) throw new Error(`unsupported totp algorithm: ${algorithm}`)
  if (!Number.isInteger(digits) || digits < 6 || digits > 8) throw new Error(`totp digits must be 6 to 8, got ${digits}`)
  if (!Number.isInteger(period) || period <= 0) throw new Error(`totp period must be a positive whole number of seconds`)
...
export function hotpCode(
  ...
  if (!Number.isInteger(digits) || digits < 6 || digits > 8) throw new Error(`hotp digits must be 6 to 8, got ${digits}`)
...
export function verifyTotp(
  ...
  if (!Number.isInteger(window) || window < 0) throw new Error('totp verification window must be a whole number of steps')
  ...
  for (let skew = -window; skew <= window; skew += 1) {
    ...
    const expected = hotpCode(key, counter, { digits: parameters.digits, algorithm: parameters.algorithm })
    ...
    if (constantTimeEquals(expected, candidate) && matched === null) matched = skew === 0 ? 0 : skew
  }
...
export function parseOtpauthUri(uri: string): TotpParameters {
  ...
  if (parsed.protocol !== OTPAUTH_SCHEME) throw new Error(`otpauth uri requires the otpauth scheme, got ${parsed.protocol}`)
...
export function decodeTotpParameters(value: unknown, id: string): TotpParameters {
  ...
  if (typeof secret !== 'string' || !secret) throw new Error(`stored secret ${id} requires a totp secret`)
