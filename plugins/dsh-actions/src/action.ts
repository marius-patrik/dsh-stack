...
function jsonRoute(parsed: { provider?: unknown; model?: unknown; route?: unknown }, filePath: string): ActionRoute | undefined {
...
export function parseAction(filePath: string, content: string): ActionSpec {
  ...
  if (extension === '.json') {
    const parsed = JSON.parse(content) as {
      name?: unknown; description?: unknown; tools?: unknown
      provider?: unknown; model?: unknown; route?: unknown; policy?: unknown
    }
    if (typeof parsed.policy !== 'string') throw new Error(`${filePath}: a JSON action needs a string "policy"`)
