...
export async function waitForServer(host: string, port: number, timeoutMs = 30_000): Promise<boolean> {
...
export function spawnReplacement(plan: RestartPlan): ChildProcess {
  const wrapper = spawn(
    plan.command,
    ['-e', HANDOFF_WRAPPER_SOURCE, plan.host, String(plan.port), plan.cwd, plan.command, ...plan.args],
...
export function createReloadHandler(options: ReloadHandlerOptions = {}) {
  ...
  return async function reloadHandler(req: ReloadRequest, res: ReloadResponse): Promise<void> {
    ...
    const spawnReplacement = options.spawnReplacement ?? ((p: RestartPlan) => spawnReplacementOf(p))
