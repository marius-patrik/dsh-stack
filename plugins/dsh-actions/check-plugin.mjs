...
let appended
agent.session = { append(type, data) { appended = { type, data } } }
const step = await preStep({ agent, signal: new AbortController().signal }, async () => ({ kind: 'enter', messages: [] }))
...
assert.deepEqual(await preExecute({ agent, name: 'bash' }, async () => ({ kind: 'allow' })), { kind: 'allow' })
assert.equal((await preExecute({ agent, name: 'write' }, async () => ({ kind: 'allow' }))).kind, 'deny')

const request = listeners.get('agent/request')
assert.deepEqual(await request({ agent }, async () => ({ provider: 'old', model: 'old' })), {
  provider: 'test-provider', model: 'test-model',
})
const rejectedAgent = { session: { events: [{ type: 'session-mode/selected', data: { mode: 'plan' } }] } }
const rejectedCommand = commands[0].handler({ agent: rejectedAgent, rawInput: 'code' })
assert.equal(rejectedCommand.kind, 'success')
const rejected = await preStep({ agent: rejectedAgent, signal: new AbortController().signal }, async () => ({ kind: 'reject' }))
...
const res = { _status: 0, _body: '', writeHead(s, h) { this._status = s; this._headers = h }, end(b) { this._body = b } }
