...
export const ACTIONS = ['tool', 'search', 'action', 'plan', 'agent', 'shell', 'code'];
...
export const DEFAULT_ACTION = 'agent';
/** The built-in actions, with their default policy texts. */
export const BUILT_IN_ACTIONS = [
    { id: 'tool', description: 'Direct tool use', policy: 'Tool action: answer through direct tool calls with minimal planning; keep steps small and explicit.' },
    { id: 'search', description: 'Research and lookup', policy: 'Search action: prioritize search and fetch tools; ground answers in sources and cite them.' },
    { id: 'action', description: 'Eager execution', policy: 'Action action: execute concrete changes eagerly; prefer doing over deliberating, and verify each change.' },
    { id: 'plan', description: 'Plan before acting', policy: 'Plan action: analyze the request and propose a plan; do not mutate files or run destructive commands until the user approves the plan.' },
    { id: 'agent', description: 'Full agent (default)', policy: 'Agent action: full autonomous multi-step execution with the complete tool surface.' },
    { id: 'shell', description: 'Shell-first operation', policy: 'Shell action: operate primarily through the shell; prefer shell pipelines over one-off file edits.' },
    { id: 'code', description: 'Code reading and editing', policy: 'Code action: focus on reading, writing, and refactoring code; keep edits minimal and typed.' },
];
/**
 ...
 */
export function sanitizeId(stem) {
    const cleaned = stem
        .toLowerCase()
        .replace(/[^a-z0-9._-]/g, '-')
        .replace(/-+/g, '-')
...
function toolList(raw) {
    if (raw === undefined)
        return undefined;
    const tools = raw.split(',').map((tool) => tool.trim()).filter((tool) => tool !== '');
    return tools.length === 0 ? undefined : tools;
}
/** A string array from a JSON action's `tools`, or undefined. */
function jsonTools(raw, filePath) {
    if (raw === undefined)
        return undefined;
    ...
    const tools = raw.map((tool) => tool.trim()).filter((tool) => tool !== '');
    return tools.length === 0 ? undefined : tools;
}
/** A route from a JSON action's `route` or `provider`/`model` pair, or undefined. */
function jsonRoute(parsed, filePath) {
 ...
 */
export function parseAction(filePath, content) {
    ...
    if (extension === '.json') {
        ...
        fields = {
            ...
            route: parsed.provider !== undefined && parsed.model !== undefined
    ...
    if (trimmed === '')
        throw new Error(`${filePath}: action policy must not be empty`);
    return {
        id,
