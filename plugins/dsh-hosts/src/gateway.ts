...
export class AccessGateway {
  ...
  start(): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.server = http.createServer((clientReq: http.IncomingMessage, clientRes: http.ServerResponse) => {
        ...
        const isHtml = clientReq.url === '/' || clientReq.url?.startsWith('/?') || clientReq.url?.includes('.html')
        ...
        proxyReq.on('error', (err: Error) => {
          if (!clientRes.headersSent) {
            ...
            clientRes.end(`dsh gateway: upstream harness unreachable on 127.0.0.1:${backendPort} (${err.message})`)
          }
        })
