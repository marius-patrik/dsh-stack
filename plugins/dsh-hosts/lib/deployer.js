...
export class RemoteDeployer {
     ...
     */
    generateUnixBootstrap(coordinatorUrl) {
...
`;
     ...
     */
    generateWindowsBootstrap(coordinatorUrl) {
...
`;
     ...
     */
    async deployToNode(node, coordinatorUrl) {
        const progress = {
            nodeId: node.id,
            stage: 'checking-ssh',
        ...
        try {
            ...
            if (!target)
                throw new Error('Target node has no reachable IP or hostname');
