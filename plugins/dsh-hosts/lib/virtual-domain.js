...
export class VirtualDomainManager {
     ...
     */
    getPermanentAddress(tailnetDns) {
    ...
    async registerMdns() {
        ...
        try {
            if (process.platform === 'darwin') {
                ...
                execFile('dns-sd', ['-R', 'dsh', '_http._tcp', 'local', String(this.config.permanentPort)], (err) => {
                    if (err)
                        console.warn('[dsh-hosts] mDNS notice:', err.message);
                });
                return true;
            }
            return false;
