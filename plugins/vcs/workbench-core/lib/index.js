...
export class ReposWorkbenchService {
    ...
    getOverview(repoPath) {
        let branch = 'main';
        let isLocalOnly = true;
        let remoteUrl;
        try {
            branch = execFileSync('git', ['branch', '--show-current'], { cwd: repoPath, encoding: 'utf-8', timeout: 3000 }).trim() || 'main';
        }
        catch { }
        try {
            remoteUrl = execFileSync('git', ['remote', 'get-url', 'origin'], { cwd: repoPath, encoding: 'utf-8', timeout: 3000 }).trim();
            isLocalOnly = !remoteUrl;
        }
        catch { }
        let uncommittedCount = 0;
        try {
            const status = execFileSync('git', ['status', '--porcelain'], { cwd: repoPath, encoding: 'utf-8', timeout: 3000 }).trim();
    ...
    registerVcsTools() {
        ...
        tools.registerTool({
            name: 'git_status',
            description: 'Get git status, modified files, and current branch for a repository (supports 100% offline local repos)',
            parameters: {
                type: 'object',
                properties: {
            ...
            execute: async (params) => {
                ...
                try {
                    const raw = execFileSync('git', ['status', '--porcelain=v1', '-b'], { cwd: repoPath, encoding: 'utf-8', timeout: 5000 }).trim();
        ...
        tools.registerTool({
            ...
            execute: async (params) => {
                ...
                try {
                    ...
                    const diff = execFileSync('git', args, { cwd: repoPath, encoding: 'utf-8', timeout: 5000 });
        ...
        tools.registerTool({
            ...
            execute: async (params) => {
                ...
                try {
                    ...
                    const out = execFileSync('git', ['commit', '-m', params.message], { cwd: repoPath, encoding: 'utf-8', timeout: 5000 }).trim();
