...
function handleMuxFrame(frame: MuxFrame, state: TuiState, sessionId: string | null): void {
  ...
  switch (frame.method) {
    ...
    case 'question/requested': {
      ...
      const question = (payload.question ?? payload.prompt ?? 'The harness is asking for input') as string
...
export async function main(): Promise<void> {
  ...
  try {
    const result = await client.call<{ sessions: Array<{ id: string; title: string; model?: string }> }>('session.list', {})
  ...
  io.on('line', async (line: string) => {
    ...
    if (cmdMatch) {
      const ctx = {
        ...
        setModelLabel: (label: string) => { state.modelLabel = label },
        setSessionLabel: (label: string) => { state.sessionLabel = label },
      }
      try {
...
export { createInitialState, render, println, printError, printInfo, printSuccess, clearScreen, ansi } from './tui.js'
