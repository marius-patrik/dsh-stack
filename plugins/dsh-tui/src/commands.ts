...
const sessionCommand: Command = {
  ...
  handler: async (args, ctx) => {
    ...
    switch (action) {
      case 'list': {
        const result = await ctx.client.call<{ sessions: Array<{ id: string; title: string; model?: string; updatedAt: string }> }>('session.list', {})
      ...
      case 'new': {
        ...
        const result = await ctx.client.call<{ id: string; title: string }>('session.create', title ? { title } : {})
      ...
      case 'switch': {
        ...
        const result = await ctx.client.call<{ sessions: Array<{ id: string; title: string }> }>('session.list', {})
...
const modelCommand: Command = {
  ...
  handler: async (args, ctx) => {
    ...
    switch (action) {
      case 'list': {
        const result = await ctx.client.call<{ models: Array<{ id: string; name: string; provider: string }> }>('session.models', { sessionId: ctx.sessionId })
  ...
  handler: async (args, ctx) => {
    ...
    switch (action) {
      case 'list': {
        ...
        const result = await ctx.client.call<{ goals?: Array<{ id: string; title: string; status: string }> }>('goal.list', { sessionId: ctx.sessionId })
      ...
      case 'create': {
        ...
        const result = await ctx.client.call<{ id: string; title: string }>('goal.create', { sessionId: ctx.sessionId, title })
