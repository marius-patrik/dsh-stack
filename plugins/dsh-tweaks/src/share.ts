...
export async function resolveSessionLogPath(home: string, sessionId: string, cwd = ''): Promise<string | undefined> {
...
export function renderSharePage(sessionId: string, lines: TranscriptLine[], interactive: boolean): string {
 ...
 * matching token.
 */
export function makeShareHandler(home: string, basePath: string, allowInteractive = false): (
  req: IncomingMessage,
  res: ServerResponse,
) => Promise<void> {
  return async (req, res) => {
    ...
    const interactive = allowInteractive && (await authorizeShare(home, url.searchParams.get('token')))
 ...
 */
export function mountShareRoute(
  ctx: Context,
  home: string,
  config: ShareConfig,
): unknown {
