...
import { normalizeSection, readTweaksSection, sectionsEqual, writeTweaksSection } from './mirror.js'
import type { TweaksSection } from './mirror.js'
import {
  NS, NS_COMMANDS, NS_KEYBINDS, NS_SESSION, NS_SHARE, NS_STATS,
  CommandsConfig, KeybindsConfig, SessionUxConfig, ShareConfig, StatsConfig,
...
import { installForkUndo, installPlanToggle, validateCommand, validateKeybinds } from './session.js'

export { normalizeSection, readTweaksSection, sectionsEqual, writeTweaksSection } from './mirror.js'
 ...
 */
export function mirrorTweaks(currentHome: string, section: () => Config, log: Pick<Context['logger'], 'warn'>): Promise<void> {
  ...
  return Promise.all([...targets].map(async (home) => {
...
function installSection<T>(
  ...
  installSettingsSection(ctx, ns, schema, entry, {
    setSource: () => { /* sections are read live through their source thunks */ },
    onChange,
...
export function apply(ctx: Context, config: Config): void {
  ...
  installSection(ctx, NS_COMMANDS, CommandsConfig, commands, (value) => {
  ...
  installSection(ctx, NS_KEYBINDS, KeybindsConfig, keybinds, (value) => {
...
function installConfiguredCommands(ctx: Context, commands: CommandsConfigType): void {
  ...
  ctx.inject(['commands'], (commandCtx) => {
    ...
    configuredCommandsDispose = () => { for (const dispose of disposers) dispose() }
