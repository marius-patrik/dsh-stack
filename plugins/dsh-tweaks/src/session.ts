...
export function forkSession(
  ...
  return { kind: 'success', text: `${direction === -1 ? 'undo' : 'redo'}: forked ${seed.length} events into a new session` }
}
...
export function validateCommand(entry: CommandEntry): void {
  if (!/^[a-z][a-z0-9_-]*$/.test(entry.name)) {
    throw new Error(`command name "${entry.name}" must be lowercase [a-z0-9_-] without a leading slash`)
  }
  if (entry.reply.trim().length === 0) throw new Error(`command "${entry.name}" has an empty reply`)
}
