...
assert.equal(defaultBaseBranch({ defaultBaseBranch: 'trunk' }, { defaultBaseBranch: 'other' }), 'trunk')
...
assert.equal(ownerRepoFromRemote('https://github.com/marius-patrik/agents.git'), 'marius-patrik/agents')
assert.equal(ownerRepoFromRemote('git@github.com:marius-patrik/agents.git'), 'marius-patrik/agents')
...
gitCtx.subprocess = {
  spawn: (spec) => {
    const res = spawnSync(spec.argv[0], spec.argv.slice(1), { cwd: spec.cwd, encoding: 'utf8', env })
    return {
      pid: res.pid ?? -1,
...
actx.provide('tools', { register: (def) => { registeredTools.push(def); return () => {} } })
...
const listed = await branchDef.execute({ path: repo, action: 'list' }, { signal: new AbortController().signal })
assert.ok(listed.branches.includes('main'))
assert.ok(listed.branches.includes('feature/one'))
await branchDef.execute({ path: repo, action: 'create', name: 'feature/two' }, { signal: new AbortController().signal })
const listed2 = await branchDef.execute({ path: repo, action: 'list' }, { signal: new AbortController().signal })
assert.ok(listed2.branches.includes('feature/two'))
await branchDef.execute({ path: repo, action: 'switch', name: 'feature/two' }, { signal: new AbortController().signal })
assert.equal(await currentBranch(gitCtx, repo), 'feature/two')
await branchDef.execute({ path: repo, action: 'switch', name: 'feature/one' }, { signal: new AbortController().signal })
await branchDef.execute({ path: repo, action: 'delete', name: 'feature/two' }, { signal: new AbortController().signal })
...
const commit = await commitDef.execute({ path: repo, message: 'add b.txt' }, { signal: new AbortController().signal })
...
accountsCtx.get = (name) => name === 'accounts' ? { resolve: async (ref) => ref === GITHUB_OAUTH_REF ? { value: vaultToken } : undefined } : undefined
...
{
  const setRes = spawnSync(process.execPath, [cli, 'set', 'remote', 'upstream'], { env: cliEnv, encoding: 'utf8' })
...
// CLI status/commit verbs over the real repo.
{
  ...
  const commitRes = spawnSync(process.execPath, [cli, 'commit', repo, 'add c.txt'], { encoding: 'utf8' })
