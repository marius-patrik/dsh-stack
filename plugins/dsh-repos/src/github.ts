 ...
 */
export async function resolveGitHubToken(ctx: Context, env: Readonly<Record<string, string | undefined>> = process.env): Promise<string | null> {
  const accounts = (ctx as { get?: (name: string) => { resolve: (ref: string) => Promise<{ value: string } | undefined> } | undefined }).get?.('accounts')
...
export async function createPullRequest(token: string, input: CreatePullRequestInput): Promise<string | null> {
