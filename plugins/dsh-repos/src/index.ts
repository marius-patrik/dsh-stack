...
async function requiredToken(ctx: Context): Promise<string> {
  ...
  if (token === null) {
    throw new Error('dsh-repos: no GitHub credential available — run `dsh accounts import` (gh hosts.yml) or set GITHUB_OAUTH_TOKEN/GH_TOKEN')
  }
  return token
}
...
export function apply(ctx: Context, config: RepoConfigType): void {
  installSettingsSection(ctx, NS, RepoSettings, { remote: 'origin', defaultBaseBranch: 'main' }, {
    setSource: () => {},
    onChange: () => {},
  })
  ...
  ctx.inject(['settings'], (sctx) => {
    ...
    ctx.tools.register(defineTool({
      name: 'repo-status',
      description: 'Show the git status of a repository: current branch, staged and unstaged changes, and untracked files.',
      parameters: {
        path: { type: 'string', description: 'Working directory of the repository (defaults to the agent cwd).' },
      },
      output: {
        schema: {
          type: 'object',
      ...
      async execute(args, exec) {
        ...
        return { path, branch: branch ?? undefined, detached: branch === null, staged, unstaged, untracked }
      },
    }))
    ...
    ctx.tools.register(defineTool({
      ...
      parameters: {
        path: { type: 'string', description: 'Working directory of the repository (defaults to the agent cwd).' },
        action: { type: 'string', enum: ['list', 'create', 'switch', 'delete'], required: true, description: 'The branch operation to perform.' },
      ...
      async execute(args, exec) {
        ...
        if (action !== 'list' && name === undefined) throw new Error(`dsh-repos: repo-branch ${action} requires a branch name`)
        ...
        if (action === 'delete') await runGit(ctx, path, ['branch', '-d', name as string], exec.signal)
    ...
    ctx.tools.register(defineTool({
      ...
      parameters: {
        ...
        paths: { type: 'array', items: { type: 'string' }, description: 'Specific paths to stage instead of everything.' },
      },
      output: {
        schema: {
          type: 'object',
        ...
        render: (_args, value) => [{ type: 'text', text: `committed ${value.commit} on ${value.branch}: ${value.summary}` }],
      },
      async execute(args, exec) {
        const path = workDir(args.path)
        ...
        const { stdout: revOut } = await runGit(ctx, path, ['rev-parse', '--short', 'HEAD'], exec.signal)
        const { stdout: summaryOut } = await runGit(ctx, path, ['show', '--stat', '--format=%s', 'HEAD'], exec.signal)
        const branch = await currentBranch(ctx, path, exec.signal)
    ...
    ctx.tools.register(defineTool({
      name: 'repo-push',
      description: 'Push the current branch (or a named branch) of a repository to its remote, authenticated with the vault GitHub token when required.',
      parameters: {
        path: { type: 'string', description: 'Working directory of the repository (defaults to the agent cwd).' },
        remote: { type: 'string', description: 'Remote name (defaults to the configured remote, usually origin).' },
        branch: { type: 'string', description: 'Branch to push (defaults to the current branch).' },
        force: { type: 'boolean', description: 'Force-push with lease (safe overwrite of the remote ref).' },
      },
      output: {
        schema: {
          type: 'object',
        ...
        render: (args, value) => [{ type: 'text', text: `pushed ${value.branch} to ${value.remote} (${value.url})` }],
      },
      async execute(args, exec) {
        const path = workDir(args.path)
        const remote = typeof args.remote === 'string' && args.remote.length > 0 ? args.remote : defaultRemote(settings(), config)
        const branch = typeof args.branch === 'string' && args.branch.length > 0 ? args.branch : (await currentBranch(ctx, path, exec.signal))
        if (branch === null) throw new Error('dsh-repos: cannot push a detached HEAD')
        const token = await requiredToken(ctx)
        await runGit(ctx, path, [
        ...
        const { stdout: urlOut } = await runGit(ctx, path, ['remote', 'get-url', remote], exec.signal)
    ...
    ctx.tools.register(defineTool({
      name: 'repo-pr',
      description: 'Open a pull request for the current branch against the repository default branch, using the vault GitHub token.',
      parameters: {
        path: { type: 'string', description: 'Working directory of the repository (defaults to the agent cwd).' },
        ...
        head: { type: 'string', description: 'The head branch (defaults to the current branch).' },
        base: { type: 'string', description: 'The base branch (defaults to the configured default, usually main).' },
      },
      output: {
        schema: {
          type: 'object',
        ...
        render: (_args, value) => [{ type: 'text', text: `PR opened: ${value.url} (${value.head} -> ${value.base} in ${value.ownerRepo})` }],
      },
      async execute(args, exec) {
        const path = workDir(args.path)
        const head = typeof args.head === 'string' && args.head.length > 0 ? args.head : (await currentBranch(ctx, path, exec.signal))
        if (head === null) throw new Error('dsh-repos: cannot open a PR from a detached HEAD')
        const base = typeof args.base === 'string' && args.base.length > 0 ? args.base : defaultBaseBranch(settings(), config)
        const remote = defaultRemote(settings(), config)
        const { stdout: urlOut } = await runGit(ctx, path, ['remote', 'get-url', remote], exec.signal)
        const ownerRepo = ownerRepoFromRemote(urlOut)
        if (ownerRepo === null) throw new Error(`dsh-repos: cannot determine owner/repo from remote ${urlOut}`)
        const token = await requiredToken(ctx)
        const url = await createPullRequest(token, { ownerRepo, head, base, title: args.title, ...args.body !== undefined ? { body: args.body } : {} })
