...
function probeBinariesAndUsage() {
  ...
  const binaries: Record<string, { installed: boolean; path: string | null; version: string | null; detail?: string }> = {}
  
  const targetBinaries = ['tmux', 'docker', 'gh', 'claude', 'cursor', 'ollama', 'git', 'node', 'python3', 'dsh']
  for (const b of targetBinaries) {
    try {
      ...
      try {
        ...
        if (b === 'tmux') {
          try { v = execSync(`${p} -V`, { encoding: 'utf-8', timeout: 1500 }).trim(); } catch {}
        }
      }
  ...
  if (binaries['tmux']?.installed) {
    try {
      const list = execSync('tmux list-sessions 2>/dev/null || true', { encoding: 'utf-8', timeout: 1200 }).trim()
  ...
  if (binaries['docker']?.installed) {
    ...
    binaries['docker'].detail = dockerRunning ? 'Daemon active & responsive' : 'CLI ready (Daemon standby)'
  }
  ...
  if (fs.existsSync(claudeStatsPath)) {
    try {
      ...
      if (dailyTokens.length > 0) {
        ...
        todayTokens = Object.values(last.tokensByModel || {}).reduce((a: number, b: any) => a + Number(b), 0)
        if (dailyTokens.length > 1) {
          const prev = dailyTokens[dailyTokens.length - 2]
          yesterdayTokens = Object.values(prev.tokensByModel || {}).reduce((a: number, b: any) => a + Number(b), 0)
        }
      }
  ...
  try {
    const rawTags = execSync('curl -s http://127.0.0.1:11434/api/tags', { encoding: 'utf-8', timeout: 1500 }).trim()
    const tagsJson = JSON.parse(rawTags)
    const rawPs = execSync('curl -s http://127.0.0.1:11434/api/ps', { encoding: 'utf-8', timeout: 1500 }).trim()
  ...
  return {
    ...
    github: { installed: binaries['gh']?.installed || false, account: ghAccount || 'marius-patrik' },
    claudeStats,
...
export function mountQuotaWeb(ctx: Context, registry: QuotaRegistry): unknown {
  return ctx.inject(['webServer'], (httpCtx) => httpCtx.webServer.register({
    kind: 'prefix', path: QUOTAS_PREFIX, handler: makeQuotaHandler(registry),
  }))
}
...
function makeQuotaHandler(registry: QuotaRegistry): (req: IncomingMessage, res: ServerResponse) => void {
  return async (req, res) => {
    ...
    try {
      ...
      if (pathname === `${QUOTAS_PREFIX}/api/tmux/sessions` && req.method === 'GET') {
        try {
          const raw = execSync('tmux list-sessions -F "#{session_name}|#{session_windows}|#{session_attached}|#{session_created}|#{pane_current_path}" 2>/dev/null || true', { encoding: 'utf-8', timeout: 1500 }).trim()
      ...
      if (pathname === `${QUOTAS_PREFIX}/api/tmux/sessions/new` && req.method === 'POST') {
        ...
        const name = String(body['name'] || `term-${Date.now().toString(36)}`).replace(/[^a-zA-Z0-9_-]/g, '')
      ...
      if (pathname === `${QUOTAS_PREFIX}/api/tmux/sessions/rename` && req.method === 'POST') {
        const body = await readBody(req)
        const oldName = String(body['oldName'] || body['name'] || '').replace(/[^a-zA-Z0-9_-]/g, '')
      ...
      if (pathname === `${QUOTAS_PREFIX}/api/tmux/sessions/capture` && req.method === 'GET') {
        ...
        try {
          ...
          const buffer = execSync(`tmux capture-pane ${flag} -t ${name}`, { encoding: 'utf-8', timeout: 2000 })
          sendJson(res, 200, { buffer })
        } catch (err) {
          sendJson(res, 200, { buffer: '(unable to capture session output)', error: (err as Error).message })
        }
        return
      }
      ...
      if (pathname === `${QUOTAS_PREFIX}/api/tmux/sessions/windows` && req.method === 'GET') {
        const name = String(url.searchParams.get('name') || '0').replace(/[^a-zA-Z0-9_-]/g, '')
        try {
          const raw = execSync(`tmux list-windows -t ${name} -F "#{window_index}|#{window_name}|#{window_active}" 2>/dev/null || true`, { encoding: 'utf-8', timeout: 1500 }).trim()
          const windows = raw ? raw.split('\n').filter(Boolean).map((line) => {
            const parts = line.split('|')
            return {
      ...
      if (pathname === `${QUOTAS_PREFIX}/api/tmux/sessions/select-window` && req.method === 'POST') {
        const body = await readBody(req)
        const name = String(body['name'] || '0').replace(/[^a-zA-Z0-9_-]/g, '')
      ...
      if (pathname === `${QUOTAS_PREFIX}/api/docker/containers` && req.method === 'GET') {
        try {
          const raw = execSync('docker ps -a --format "{{.ID}}|{{.Image}}|{{.Status}}|{{.Names}}|{{.Ports}}|{{.Mounts}}|{{.Labels}}" 2>/dev/null || true', { encoding: 'utf-8', timeout: 2000 }).trim()
      ...
      if (pathname === `${QUOTAS_PREFIX}/api/docker/containers/logs` && req.method === 'GET') {
        ...
        if (id) {
          try {
            const logs = execSync(`docker logs --tail 100 ${id} 2>&1`, { encoding: 'utf-8', timeout: 3000 })
            sendJson(res, 200, { logs })
          } catch (err) {
            sendJson(res, 200, { logs: '(unable to fetch container logs)', error: (err as Error).message })
          }
        } else {
      ...
      if (pathname === `${QUOTAS_PREFIX}/api/fs` && req.method === 'GET') {
        ...
        try {
          ...
          if (!stat.isDirectory()) {
            sendJson(res, 200, { path: targetPath, isFile: true, size: stat.size, mtime: stat.mtimeMs, entries: [] })
            return
          }
          const rawEntries = fs.readdirSync(targetPath, { withFileTypes: true })
          const entries = []
          for (const dirent of rawEntries) {
            if (dirent.name.startsWith('.') && dirent.name !== '.cursor' && dirent.name !== '.agents') continue
            try {
              const full = path.join(targetPath, dirent.name)
              const isDir = dirent.isDirectory()
              const isVendor = dirent.name === 'node_modules' || dirent.name === 'dist' || dirent.name === 'lib' || dirent.name === '.git'
              let isRepo = false
              if (isDir && !isVendor) {
                try {
          ...
          sendJson(res, 200, { path: targetPath, parent: null, entries: [], error: (err as Error).message })
        }
        return
      }
      ...
      if (pathname === `${QUOTAS_PREFIX}/api/summary` && req.method === 'GET') {
        ...
        const summary = {
          ...
          exhausted: snapshots.filter((s) => s.status === 'available' && s.remaining !== undefined && s.remaining <= 0).length,
...
function renderDashboard(snapshots: readonly QuotaSnapshot[]): string {
  const rows = snapshots.map((s: QuotaSnapshot) => {
    const meter = s.used !== undefined && s.limit !== undefined
      ? meterBar(s.used, s.limit)
      : s.status === 'available' ? 'available' : s.status === 'error' ? `\x1b[31merror\x1b[0m` : 'unknown'
