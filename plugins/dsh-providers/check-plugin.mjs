...
if (providers.default !== undefined) throw new Error('function plugins must not have a default export')
assert.deepEqual(providers.PROVIDER_IDS, [
  'kimi-code', 'kimi-sub', 'claude-sub', 'grok-sub', 'gemini-sub', 'antigravity-sub',
  'openai-api', 'anthropic-api', 'gemini-api', 'grok-api',
  'deepseek-api', 'mistral-api', 'groq-api', 'openrouter-api',
  'zen', 'ollama', 'llamacpp', 'vllm',
])
...
const llm = {
  ...
  registerConfigurableProviders(entries) { this.configurable.push(...entries) },
...
assert.deepEqual(llm.configurable.map((p) => p.provider), providers.PROVIDER_IDS)
assert.deepEqual(llm.registeredProviders, providers.PROVIDER_IDS)
console.log('registration ok:', llm.configurable.map((p) => `${p.provider}=${p.displayName}`).join(', '))
...
await assert.rejects(() => adapter.resolveModel('grok-sub', 'grok-4.6'), { code: 'MISSING_CREDENTIAL' })
const models = await adapter.listModels('claude-sub')
assert.ok(models.some((m) => m.id === 'claude-sonnet-5'))
assert.ok(!providers.PROVIDER_ROUTES.some((r) => r.id === 'cursor-sub'), 'cursor-sub must be dropped')
const resolved = await adapter.resolveModel('claude-sub', 'claude-sonnet-5')
assert.equal(resolved.defaultMaxTokens, 128_000)
console.log('subscription-only filter ok: kimi-code hidden, grok-sub hidden without a login, claude-sub usable')
...
console.log('catalog ok (all mode):', (await openAdapter.listModels('claude-sub')).map((m) => m.id).join(', '))
...
console.log('claude-sub stream ok:', JSON.stringify({ url: capturedUrl, text, usage: usage.usage }))
...
assert.ok(openaiText.includes('Hello world'), `unexpected assembled text: ${JSON.stringify(openaiText)}`)
...
assert.ok(geminiText.includes('Hello world'), `unexpected gemini text: ${JSON.stringify(geminiText)}`)
const geminiUsage = geminiChunks.find((c) => c.usage !== undefined)
assert.ok(geminiUsage, 'no gemini usage chunk')
console.log('gemini-sub stream ok:', JSON.stringify({ url: capturedUrl, text: geminiText, usage: geminiUsage.usage }))
...
const kimiQuotaDetail = 'access_terminated_error You\'ve reached your usage limit for this'
...
assert.equal(providers.httpErrorCode(403, 'insufficient_quota You exceeded your current quota'), 'QUOTA')
...
assert.equal(providers.httpErrorCode(401, 'CreditsError No payment method. Add a payment method here: https://…'), 'QUOTA')
...
// ---- the credential gate keeps unusable rows out of the selector ----
{
  ...
  const llmBare = {
    ...
    registerAdapter(registered, adapter) { this.adapter = adapter; return { replace: () => {}, dispose: () => {} } },
  }
  ctxBare.provide('llm', llmBare)
  ctxBare.provide('settings', settings)
  // Nothing is configured: every route is uncredentialed.
  ctxBare.provide('credentials', { async resolve() { return undefined } })
  ...
  for (const provider of ['kimi-code', 'kimi-sub', 'openai-api', 'zen']) {
    ...
    assert.deepEqual(await ctxBare.llm.adapter.listModels(provider), [], `${provider} must list nothing`)
    await assert.rejects(() => ctxBare.llm.adapter.resolveModel(provider, 'anything'), { code: 'MISSING_CREDENTIAL' })
  }
  ...
  globalThis.fetch = async (url) => {
    if (String(url).includes('/oauth/token')) return new Response('{"error":"invalid_grant"}', { status: 400 })
...
// ---- live model discovery ----
{
  ...
  assert.deepEqual(parseCatalogResponse({
    data: [
  ...
  const merged = mergeCatalog(
    ...
    [{ id: 'known', name: 'Known', contextWindow: 111, maxTokens: 22 }, { id: 'alias', name: 'Alias', contextWindow: 333, maxTokens: 44 }],
  ...
  const okCatalog = new ModelCatalog({
    fetch: async (url, init) => {
      ...
      return new Response(JSON.stringify({ data: [{ id: 'fresh-model', context_length: 42 }] }), { status: 200 })
    },
  })
  ...
  assert.deepEqual(first.map((m) => m.id), ['fresh-model', 'static-only'])
  assert.deepEqual(second, first)
  assert.deepEqual((await okCatalog.models('p', request)).map((m) => m.id), ['fresh-model', 'static-only'])
  ...
  assert.deepEqual((await refused.models('p', request)).map((m) => m.id), ['static-only'])
  const offline = new ModelCatalog({ fetch: async () => { throw new Error('offline') } })
  assert.deepEqual((await offline.models('p', request)).map((m) => m.id), ['static-only'])
  ...
  await anthropic.models('a', { ...request, source: { url: 'https://example.test/v1/models', authStyle: 'x-api-key' } })
  ...
  await queryAuth.models('g', { ...request, source: { url: 'https://example.test/v1beta/models', authStyle: 'query' } })
...
// changer from the picker the moment discovery came online.
  const withEfforts = mergeCatalog(
    [{ id: 'known' }],
    [{ id: 'known', name: 'Known', contextWindow: 1, maxTokens: 1, reasoning: { efforts: [{ id: 'high', name: 'High' }] } }],
...
{
  const {
    createConfiguredProviders, probeConfiguredRoute, readConfiguredProfile, modelsEndpoint, PROBE_ROUTE_IDS,
  } = providers
  ...
  const descriptors = [{
    ns: 'llm-pi-ai',
    value: { providers: { 'my-gateway': { baseURL: 'https://gw.test/v1', apiKeyEnv: 'MY_GW_KEY' } } },
  }]
  const entry = {
    provider: 'my-gateway',
  ...
  assert.equal(readConfiguredProfile({ ...entry, settingsPath: ['providers', 'other'] }, descriptors), undefined)

  // Each status the endpoint can answer with maps to the light it deserves.
  const statuses = [[200, 'available'], [401, 'error'], [403, 'error'], [429, 'error'], [404, 'unknown'], [500, 'unknown']]
  for (const [status, expected] of statuses) {
    const snap = await probeConfiguredRoute(
      entry, { baseURL: 'https://gw.test/v1', apiKeyEnv: 'MY_GW_KEY' }, 'secret',
  ...
  let sentAuth
  await probeConfiguredRoute(entry, { baseURL: 'https://gw.test/v1', apiKeyEnv: 'MY_GW_KEY' }, 'secret',
    async (url, init) => { sentAuth = init.headers['authorization']; return new Response('', { status: 200 }) })
  assert.equal(sentAuth, 'Bearer secret')
  const noCred = await probeConfiguredRoute(entry, { baseURL: 'https://gw.test/v1', apiKeyEnv: 'MY_GW_KEY' }, undefined,
  ...
  const keyless = await probeConfiguredRoute(entry, { baseURL: 'http://127.0.0.1:11434/v1' }, undefined,
    async (url, init) => { keylessAuth = init.headers['authorization']; return new Response('', { status: 200 }) })
  ...
  const noEndpoint = await probeConfiguredRoute(entry, {}, undefined, async () => { throw new Error('must not be called') })
  ...
  assert.deepEqual(built.map((p) => p.id), ['my-gateway'])
  ...
  assert.deepEqual(unconfigured.map((p) => p.id), ['my-gateway'])
