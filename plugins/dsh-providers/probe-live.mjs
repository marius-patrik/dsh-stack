...
const llm = {
  configurable: [], adapter: undefined,
  registerConfigurableProviders(entries) { this.configurable.push(...entries) },
  registerAdapter(_ids, adapter) { this.adapter = adapter; return { replace: () => {}, dispose: () => {} } },
}
ctx.provide('llm', llm)
ctx.provide('settings', { register: (_ns, _s, opts) => ({ get: () => opts.base, watch: () => undefined }) })
...
const modelOverride = modelArg ? (() => { const s = modelArg.slice(8); const i = s.indexOf(':'); return [s.slice(0, i), s.slice(i + 1)] })() : null
const firstModel = async (id) => {
  ...
  try {
    ...
    if (route?.models?.[0]?.id) return { model: route.models[0].id, via: 'static (discovery empty — gated?)' }
    return { model: undefined, via: 'none' }
  } catch (e) {
    if (route?.models?.[0]?.id) return { model: route.models[0].id, via: 'static (discovery: ' + String(e?.code ?? e?.message ?? e).slice(0, 60) + ')' }
...
for (const id of ids) {
  ...
  try {
    ...
    const timeout = new Promise((_, rej) => setTimeout(() => rej(new Error('probe timeout ' + timeoutMs / 1000 + 's')), timeoutMs))
    const run = (async () => {
  ...
  console.log(mark + ' ' + id + ' (' + model + ', ' + row.via + ') ' + row.ms + 'ms' + (row.text ? ' — "' + row.text + '"' : '') + (row.error ? ' — ' + row.error : '') + (row.gate ? ' [gate: ' + row.gate + ']' : ''))
}
