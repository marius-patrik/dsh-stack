...
const vsSource = theme.parseVsCodeTheme(JSON.stringify({
  name: 'Monokai Pro',
  type: 'dark',
  colors: {
...
assert.equal(vsDef.tokens['--dsw-alias-state-success-primary'], theme.TOKEN_FALLBACKS['--dsw-alias-state-success-primary'].dark)
assert.equal(Object.keys(vsDef.tokens).length, 13)
console.log('vs-code map ok:', vsDef.id)

// theme: light scheme + rgb-normalization.
const lightDef = theme.mapTheme(theme.parseVsCodeTheme(JSON.stringify({
  name: 'Paper',
  type: 'light',
...
const tmDef = theme.mapTheme(theme.parseTmThemeXml(`
  <dict>
...
writeFileSync(join(extRoot, 'extension', 'themes', 'aurora.json'), JSON.stringify({
  name: 'Aurora',
  type: 'dark',
...
const catalogServer = createServer((req, res) => {
  if (req.url.includes('/api/-/search')) {
    ...
    res.end(JSON.stringify({ extensions: [{
      namespace: 'octref', name: 'theme-monokai-pro', displayName: 'Monokai Pro',
      description: 'A theme', version: '1.0.0', downloadCount: 123,
...
const clientCtx = {
  effect(fn) { fn() },
  on() { return () => undefined },
  theme: { getTheme: () => ({ active: { id: 'light' }, themes: [] }), setTheme() {}, register() { return () => undefined } },
  slots: {
