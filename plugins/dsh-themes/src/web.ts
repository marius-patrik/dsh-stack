...
import { downloadVsix, extractThemesFromVsix, resolveCatalogExtension, searchCatalog } from './catalog.js'
...
export function mountThemeWeb(ctx: Context, deps: ThemeWebDeps): unknown {
  return ctx.inject(['webServer'], (httpCtx) => httpCtx.webServer.register({
    kind: 'prefix', path: THEMES_API_PREFIX, handler: makeThemeHandler(deps),
  }))
}
 ...
 */
async function installCatalogExtension(deps: ThemeWebDeps, namespace: string, name: string): Promise<StoredTheme[]> {
...
function makeThemeHandler(deps: ThemeWebDeps): (req: IncomingMessage, res: ServerResponse) => Promise<void> {
