...
export function parseVsCodeTheme(text: string, nameOverride?: string): ThemeSource {
  ...
  const colors = json.colors !== null && typeof json.colors === 'object'
...
export function parseTmThemeXml(text: string): ThemeSource {
  ...
  const settingsMatch = /<key>\s*settings\s*<\/key>\s*<dict>([\s\S]*?)<\/dict>\s*<\/dict>/.exec(text)
...
function inferScheme(colors: Record<string, string>): ThemeSource['type'] {
  ...
  if (rgb === null || rgb[1] === undefined || rgb[2] === undefined || rgb[3] === undefined) return 'dark'
  const luminance = (parseInt(rgb[1], 16) * 0.299 + parseInt(rgb[2], 16) * 0.587 + parseInt(rgb[3], 16) * 0.114)
