window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function ThemesGlyph() {
			...
			return React.createElement(P.IconLightOutline16, { size: 16, className: "dsh-themes-navGlyph" });
		}

		function ThemesSection(props) {
			var React = require("react");
			var h = React.createElement;
			var useThemeSnapshot = props.useThemeSnapshot, applyTheme = props.applyTheme;
			var snapshot = useThemeSnapshot(function (s) { return s; });
			...
			var borderStyle = borderStyleState[0], setBorderStyle = borderStyleState[1];
			...
			var showHeroBanner = heroBannerState[0], setShowHeroBanner = heroBannerState[1];
			...
			var setBorderChoice = function (style) {
				...
				if (typeof window !== 'undefined') window.dispatchEvent(new CustomEvent('dsh:settings-change'));
			};
			...
			var themeButton = function (theme) {
				...
				return h("button", {
					...
					style: {
						...
						border: isActive ? "1px solid var(--dsw-alias-border-accent)" : "1px solid var(--dsw-alias-border-l2)",
						borderRadius: "10px", padding: "12px 14px", background: isActive ? "var(--dsw-specific-sidebar-nav-item-active)" : "transparent",
						cursor: "pointer", fontFamily: "inherit", fontSize: "14px", color: "var(--dsw-alias-label-primary)", textAlign: "left",
					},
				},
					h("span", { style: { width: "12px", height: "12px", borderRadius: "3px", flex: "none", background: theme.colorScheme === "dark" ? "#1f2430" : "#ffffff", border: "1px solid var(--dsw-alias-border-l2)" } }),
					h("span", null, theme.id),
					isActive ? h("strong", { style: { marginLeft: "auto", fontSize: "12px" } }, "Active") : null);
			};
			var schemeGroup = function (scheme) {
				var list = groups[scheme];
				if (!list || list.length === 0) return null;
				return h("div", { key: scheme, style: { display: "grid", gap: "8px" } },
					h("div", { style: { fontSize: "13px", fontWeight: 600, color: "var(--dsw-alias-label-secondary)" } },
						scheme === "light" ? "Light Palettes" : "Dark Palettes"),
					h("div", { style: { display: "grid", gap: "8px", gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))" } },
			...
			var modeButtons = h("div", { style: { display: "flex", flexDirection: "column", gap: "10px", paddingBottom: "16px", borderBottom: "1px solid var(--dsw-alias-border-l1)" } },
				h("div", { style: { fontSize: "13px", fontWeight: 600, color: "var(--dsw-alias-label-primary)" } }, "Theme Mode"),
				h("div", { style: { display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: "10px" } },
					[
						...
						var isSelected = mode.id === "oled" ? isOledActive : (mode.id === "system" ? false : (mode.id === currentScheme && !isOledActive));
						return h("button", {
							key: mode.id,
							type: "button",
							onClick: function () {
								...
								var targetTheme = mode.id === "light"
									? (groups["light"] && groups["light"][0] ? groups["light"][0].id : "light")
									: (groups["dark"] && groups["dark"][0] ? groups["dark"][0].id : "dark");
								applyTheme(targetTheme);
							},
							style: {
								display: "flex", flexDirection: "column", gap: "4px", padding: "12px 14px", borderRadius: "10px",
								border: isSelected ? "1px solid var(--dsw-alias-primary, #6366f1)" : "1px solid var(--dsw-alias-border-l2)",
								background: isSelected ? "rgba(99, 102, 241, 0.1)" : "var(--dsw-alias-surface-l1, rgba(128,128,128,0.03))",
								cursor: "pointer", textAlign: "left", color: "inherit",
							},
						},
							h("div", { style: { fontSize: "14px", fontWeight: 600, color: isSelected ? "var(--dsw-alias-primary, #6366f1)" : "var(--dsw-alias-label-primary)" } }, mode.label),
							h("div", { style: { fontSize: "11px", color: "var(--dsw-alias-label-secondary)" } }, mode.desc));
					})
				)
			);

			var borderButtons = h("div", { style: { display: "flex", flexDirection: "column", gap: "10px", paddingBottom: "16px", borderBottom: "1px solid var(--dsw-alias-border-l1)" } },
				h("div", { style: { fontSize: "13px", fontWeight: 600, color: "var(--dsw-alias-label-primary)" } }, "Border Style"),
				h("div", { style: { display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: "10px" } },
					[
						...
						return h("button", {
							key: b.id,
							type: "button",
							onClick: function () { setBorderChoice(b.id); },
							style: {
								display: "flex", flexDirection: "column", gap: "4px", padding: "12px 14px", borderRadius: "10px",
								border: isSelected ? "1px solid var(--dsw-alias-primary, #6366f1)" : "1px solid var(--dsw-alias-border-l2)",
								background: isSelected ? "rgba(99, 102, 241, 0.1)" : "var(--dsw-alias-surface-l1, rgba(128,128,128,0.03))",
							...
							h("div", { style: { fontSize: "14px", fontWeight: 600, color: isSelected ? "var(--dsw-alias-primary, #6366f1)" : "var(--dsw-alias-label-primary)" } }, b.label),
							h("div", { style: { fontSize: "11px", color: "var(--dsw-alias-label-secondary)" } }, b.desc));
					})
				)
			);

			var bannerSection = h("div", {
				style: {
					display: "flex", alignItems: "center", justifyContent: "space-between",
					padding: "14px 16px", borderRadius: "10px", border: "1px solid var(--dsw-alias-border-l2)",
					background: "var(--dsw-alias-surface-l1, rgba(128,128,128,0.03))",
				}
			},
				h("div", { style: { display: "flex", flexDirection: "column", gap: "3px" } },
					h("div", { style: { fontSize: "14px", fontWeight: 600, color: "var(--dsw-alias-label-primary)" } }, "Into the Unknown Greeting Banner"),
					h("div", { style: { fontSize: "12px", color: "var(--dsw-alias-label-secondary)" } }, "Show the illustration and headline banner on new conversation screens")
				),
				h("button", {
					type: "button",
					...
					style: {
						...
						border: "none", background: showHeroBanner ? "var(--dsw-alias-primary, #6366f1)" : "var(--dsw-alias-border-l2, #444)",
						cursor: "pointer", transition: "background 150ms ease",
					}
				},
					h("span", {
						style: {
							position: "absolute", top: "2px", left: showHeroBanner ? "22px" : "2px",
							width: "20px", height: "20px", borderRadius: "50%", background: "#ffffff",
							boxShadow: "0 1px 3px rgba(0,0,0,0.3)", transition: "left 150ms ease",
						}
					})
				)
			);
			...
			return h("div", { style: { display: "grid", gap: "20px", maxWidth: "760px" } },
				h("div", null,
					...
					h("p", { style: { margin: 0, color: "var(--dsw-alias-label-secondary)" } }, "Customize visual styles, theme modes, borders, and layout preferences.")),
				modeButtons,
				borderButtons,
				bannerSection,
		...
		function apply(ctx) {
			...
			ctx.effect(() => {
				...
				return () => { themeObservable.dispose(); };
			...
			ctx.effect(() => {
				...
				try {
					...
					if (savedTheme && (savedTheme === 'oled' || savedTheme === 'light' || savedTheme === 'dark' || savedTheme === 'system')) {
						try {
				...
				var cancel = fetch(THEMES_ROUTE).then((response) => {
					if (!response.ok) throw new Error(`dsh-themes: ${THEMES_ROUTE} failed (HTTP ${response.status})`);
					...
					for (var i = 0; i < (body.themes || []).length; i++) {
						...
						known.add(item.id);
						registrations.push(theme.register({
							id: item.id,
					...
					if (targetTheme && (targetTheme === 'system' || targetTheme === 'light' || targetTheme === 'dark' || known.has(targetTheme))) {
						try {
			...
			ctx.slots.inject("settings.section", () => ctx.slots.register({
				...
				inject: () => ({
					...
					applyTheme: (id) => {
						...
						try { ctx.theme.setTheme(id); } catch (err) { console.error('[dsh-themes] failed to set theme:', err); }
					},
				}),
			}, ThemesSection), "dsh-themes: themes settings section");
			ctx.slots.inject("settings.section.icon", () => ctx.slots.register({
				name: "settings.section.icon",
				id: "themes",
				order: 0,
			}, ThemesGlyph), "dsh-themes: themes nav glyph");
		}
		//#endregion
		exports.apply = apply;
