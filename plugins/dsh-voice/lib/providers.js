 ...
 */
export function authHeaders(auth, key) {
    if (key.length === 0 || auth === 'none')
        return {};
    if (auth === 'api-key')
        return { 'api-key': key };
 ...
 */
export function resolveTts(settings) {
    ...
    if (baseURL.length === 0) {
        throw new Error(`dsh-voice: provider "${provider.id}" needs a base URL — set voice.tts.apiBase (e.g. https://api.openai.com/v1 or your gateway)`);
    }
