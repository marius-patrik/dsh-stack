...
function sendError(res, err, notConfiguredStatus = 400) {
    const e = err;
 ...
 */
export function makeTtsHandler(current, accounts) {
    return async (req, res) => {
        try {
            ...
            const overrides = {
                ...(typeof payload.voice === 'string' && payload.voice.length > 0 ? { voice: payload.voice } : {}),
                ...(typeof payload.speed === 'number' && Number.isFinite(payload.speed) ? { speed: payload.speed } : {}),
                ...(typeof payload.format === 'string' && payload.format.length > 0 ? { format: payload.format } : {}),
            };
 ...
 * @returns The webServer route handler.
 */
export function makeSttHandler(current, accounts) {
    return async (req, res) => {
        try {
            if (req.method !== 'POST') {
                sendJson(res, 405, { error: 'method not allowed' });
 ...
 * @returns The webServer route handler.
 */
export function makeConfigHandler(current) {
    return (req, res) => {
        if (req.method !== 'GET') {
            sendJson(res, 405, { error: 'method not allowed' });
            return;
