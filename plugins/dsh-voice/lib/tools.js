...
import { resolveCredential, synthesizeSpeech, transcribeAudio } from './speech.js';
/**
 ...
 */
export function registerVoiceTools(tools, current, accounts) {
    tools.register(defineTool({
        name: 'voice_speak',
        description: 'Generate natural, human-sounding spoken audio from text via the configured neural TTS endpoint (OpenAI-compatible /audio/speech, e.g. gpt-4o-mini-tts with voices like nova or shimmer). Writes the audio file into the workspace and returns its path.',
        parameters: {
            text: {
                type: 'string',
                required: true,
        ...
        output: {
            ...
            render: (_args, value) => [
                { type: 'text', text: `Generated speech: ${value.path} (${value.bytes} bytes, voice ${value.voice})` },
            ],
        },
        async execute(args) {
            const config = current();
            if (!config.tts.enabled)
                throw new Error('TTS is disabled (voice.tts.enabled)');
            ...
            if (apiKey.length === 0 && target.provider.auth !== 'none') {
                throw new Error(`voice_speak needs a credential: store ${target.credentialRef} in the dsh account vault (or export it in the environment), or point voice.tts at a keyless local endpoint`);
            }
            const overrides = {
            ...
            const out = args.outPath !== undefined
    ...
    tools.register(defineTool({
        ...
        description: 'Transcribe an audio file (wav/mp3/m4a/ogg/webm/flac) to text via a Whisper-compatible /audio/transcriptions endpoint. Lets a text-only model hear recordings the user attaches.',
        parameters: {
            path: {
                type: 'string',
                required: true,
        ...
        async execute(args) {
            ...
            if (!config.stt.enabled)
                throw new Error('STT is disabled (voice.stt.enabled)');
