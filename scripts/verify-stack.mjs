...
async function trackedGeneratedFiles() {
  try {
    const { stdout } = await execFileAsync('git', ['ls-files', '--', 'packages/**/lib/**', 'packages/**/dist/**', 'packages/**/node_modules/**', 'plugins/**/lib/**', 'plugins/**/dist/**', 'plugins/**/node_modules/**'], { cwd: root })
...
async function verifyCanonicalPackage(dir) {
  ...
  assert(typeof packageManifest?.name === 'string' && packageManifest.name.length > 0, `${relative(root, packagePath)} must declare a package name`)
  ...
  for (const publishedPath of stack.files) {
    ...
    assert(await exists(join(dir, publishedPath)), `${stack.id} publishes missing path ${publishedPath}`)
  }
}
...
async function verifyPluginTree() {
  ...
  for (const child of children) {
    ...
    assert(manifest?.private === true, `${relative(root, packagePath)} must be a private composition wrapper`)
    assert(manifest?.type === 'module', `${relative(root, packagePath)} must use ESM`)
    assert(await exists(join(dir, 'src', 'index.mjs')), `${relative(root, dir)} must import its canonical package through src/index.mjs`)
    const source = await fs.readFile(join(dir, 'src', 'index.mjs'), 'utf8')
    assert(source.includes("../../../packages/"), `${relative(root, dir)}/src/index.mjs must resolve its canonical package through packages/`)
  }
}
...
async function main() {
  ...
  for (const child of packageChildren) {
    ...
    assert(await exists(join(packagesDir, child.name, 'package.json')), `packages/${child.name} must contain package.json`)
  ...
  for await (const file of walk(packagesDir)) {
    ...
    assert(!/(?:from\s+|import\s*\()(['"]).*plugins\//.test(text), `${rel} imports implementation code from plugins/`)
  ...
  console.log(`Stack verification passed: ${publicPackages.size} public packages, ${packageNames.size} canonical manifests, ${sourceHashes.size} unique implementation source bodies.`)
}
