...
async function main() {
  ...
  for (const child of children) {
    ...
    if (typeof stack.id === 'string') {
      ...
      else stackIds.set(stack.id, relative(root, stackPath))
    }
    packages.set(stack.id, { dir, stack })
    ...
    if (await exists(packagePath)) {
      ...
      if (manifest?.name) {
        ...
        if (previous) fail(`duplicate package name ${manifest.name}: ${previous} and ${relative(root, packagePath)}`)
        else packageNames.set(manifest.name, relative(root, packagePath))
      }
      assert(manifest?.type === 'module', `${relative(root, packagePath)} must use ESM`)
      assert(manifest?.private !== true, `${relative(root, packagePath)} is private; public Stack packages must be publishable`)
      assert(manifest?.stack?.id === stack.id, `${relative(root, packagePath)} stack.id does not match stack.json`)
    }
  }
  ...
  console.log(`Stack verification passed: ${packages.size} packages, ${packageNames.size} publishable manifests, ${sourceHashes.size} unique source bodies.`)
}
